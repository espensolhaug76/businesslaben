...
type KonkurranserFilter = 'alle' | 'mitt-fag' | 'ssr' | 'ml' | 'ent'
...
function KonkurranserTab({ navigate }: { navigate: (path: string) => void }) {
  ...
  const [filter, setFilter] = useState<KonkurranserFilter>('alle')

  // Lærerens egne fag (fra teacher-classes) — for "Mitt fag"-filteret
  const ownSubjects = (() => {
    try {
    ...
    <div>
      ...
      {(() => {
        const filtered = competitions.filter(c => {
          const sub = normalizeSubjectId(c.subject)
          if (filter === 'mitt-fag') return sub && ownSubjects.has(sub)
        ...
        if (filtered.length === 0 && competitions.length > 0) {
          return (
