...
const TERMS: Record<string, string> = {
  'Selvkost': 'Det en vare faktisk koster bedriften å produsere eller kjøpe inn.',
  'Fortjeneste': 'Differansen mellom salgspris og selvkost – det bedriften tjener per vare.',
  'Kostnadsbasert prising': 'Prissetting der man legger fortjeneste oppå selvkost for å finne salgsprisen.',
  'Markedsbasert prising': 'Prissetting basert på hva kundene er villige til å betale for produktet.',
  'Konkurransebasert prising': 'Prissetting der man tilpasser seg hva konkurrentene tar for tilsvarende produkter.',
  'Psykologisk prissetting': 'Prissetting som utnytter psykologi, f.eks. 199,- i stedet for 200,-.',
  'Yield Management': 'Dynamisk prissetting der prisen endres basert på etterspørsel og tidspunkt.',
  'Verdibasert prissetting': 'Prissetting basert på den opplevde verdien kunden får, ikke bare kostnadene.',
}
...
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    ...
    options: [
      'Pris satt ut fra hva konkurrentene tar i markedet',
      'Pris satt ut fra hva kundene maksimalt er villige til å betale',
      'Pris beregnet som produksjonskostnad pluss ønsket fortjeneste',
      'Pris justert dynamisk basert på etterspørsel og tid',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  {
    question: 'Hva er psykologisk prissetting?',
    options: [
      'Prissetting basert på psykologisk analyse av kundesegmenter',
      'Å sette priser som 199 kr i stedet for 200 kr for å virke rimeligere',
      'Å justere priser basert på kundens følelsesmessige tilstand',
      'Å skjule de reelle kostnadene i tjenestepakker',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Fast prissetting likt for alle kunder uavhengig av tidspunkt',
      'Dynamisk prissetting som varierer basert på etterspørsel og kapasitet',
      'Rabattsystem for lojale og faste kunder',
      'Prisøkning uavhengig av markedssituasjonen',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Den prisen kunden betaler for et produkt',
      'Det en vare faktisk koster bedriften å produsere eller kjøpe inn',
      'Fortjenesten bedriften oppnår ved salg',
      'Merverdiavgiften som legges på salgsprisen',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Pris beregnet ut fra produksjonskostnader og ønsket fortjeneste',
      'Pris basert på hva konkurrentene tar i markedet',
      'Pris satt ut fra den opplevde verdien kunden får, ikke bare kostnadene',
      'Pris justert automatisk basert på etterspørselssvingninger',
    ],
    correct: 2,
    timeSeconds: 45,
  },
...
function Slide1({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 4 – Prissetting</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Prisen sender en melding
          </h2>
          <p style={bodyText}>
            Hvis du ser en hvit t-skjorte til 50 kr og en annen til 800 kr, tenker du automatisk at den dyre har bedre kvalitet eller er "kulere". Pris forteller kunden hvem du er.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-6726b3ff858f?auto=format&fit=crop&w=800&q=80" alt="Prislapp" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Kalkulator" />
        <div>
          <SlideLabel>Prismetode</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Kostnadsbasert prising" onShow={onShow} />
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Du regner ut hva varen koster deg (<Term word="Selvkost" onShow={onShow} />) og legger på de pengene du vil tjene selv (<Term word="Fortjeneste" onShow={onShow} />):
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <p style={{ ...bodyText, color: '#38bdf8', fontWeight: 700, textAlign: 'center', fontSize: 20 }}>
              Salgspris = Selvkost + Fortjeneste
            </p>
          </div>
        </div>
      </div>
    ...
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Prismetode</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Markedsbasert prising" onShow={onShow} /> – Hvor mye er du villig til å betale?
          </h2>
          <p style={bodyText}>
            Hvis du er kjempesulten på en konsert, betaler du gjerne 100 kr for en pølse. Prisen settes etter hvor mye kunden "må" ha varen.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?auto=format&fit=crop&w=800&q=80" alt="Konsert mat" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-6726b3ff858f?auto=format&fit=crop&w=800&q=80" alt="Butikk" />
        <div>
          <SlideLabel>Prismetode</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Konkurransebasert prising" onShow={onShow} />
          </h2>
          <p style={bodyText}>
            Du ser hva de andre tar. Hvis Kiwi og Rema har ulik pris på samme melk, mister den dyreste butikken alle kundene.
          </p>
        </div>
      </div>
    </Glass>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Psykologi i prissetting</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Psykologisk prissetting" onShow={onShow} /> – Hjernetrikset
          </h2>
          <p style={bodyText}>
            Hvorfor koster alt 199,- og ikke 200,-? Fordi hjernen vår fokuserer på det første tallet (1), og det føles mye billigere enn 2.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?auto=format&fit=crop&w=800&q=80" alt="Psykologi" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Fly" />
        <div>
          <SlideLabel>Dynamisk prissetting</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Yield Management" onShow={onShow} /> – Pris som endrer seg
          </h2>
          <p style={bodyText}>
            Flybilletter er dyre rett før avreise fordi bedriftskunder må dra, men billige tre måneder før for å lokke ferieturister.
          </p>
        </div>
      </div>
    </Glass>
      ...
      <div style={twoCol}>
        <div>
          ...
          <div style={{ ...bodyText, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem', marginBottom: 16 }}>
            De bruker <Term word="Verdibasert prissetting" onShow={onShow} />. De tar en høy pris fordi kundene føler at de kjøper noe mer enn bare is; de støtter rettferdig handel og miljøsaker.
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-6726b3ff858f?auto=format&fit=crop&w=800&q=80" alt="Is" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Er det greit at Uber tar dobbel pris når det regner og alle vil ha bil samtidig?
        </h2>
        <p style={{ ...bodyText, fontSize: 20 }}>
          Tenk på hvem som tjener og hvem som taper. Er dynamisk prissetting rettferdig, eller utnytter det folk i nød?
        </p>
      </div>
    </Glass>
  )
...
export default function PrissettingPresentation() {
