...
const TERMS: Record<string, string> = {
  'GDPR': 'EUs personvernforordning (Personopplysningsloven) som gir individet kontroll over egne data og stiller krav til bedrifter.',
  'Phishing': 'Sosial manipulering via e-post eller SMS for å stjele passord, bankinformasjon eller bedriftshemmeligheter.',
  'Behandlingsgrunnlag': 'Det lovlige mandatet for å samle inn og behandle persondata — f.eks. samtykke eller avtale.',
  'Informasjonssikkerhet': 'Tekniske og menneskelige tiltak for å sikre konfidensialitet, integritet og tilgjengelighet til data.',
  'Sletterutine': 'Standardiserte prosedyrer for å fjerne personopplysninger når formålet med lagringen har opphørt.',
  'Dataminimering': 'GDPR-prinsippet om at man kun skal samle inn data som er nødvendig for et spesifikt formål.',
}

const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva forbyr GDPR?',
    options: [
      'All oppbevaring av kundedata',
      'Kun lagring av sensitive helseopplysninger',
      'Formålsløs innsamling av unødvendig persondata',
      'Bruk av e-post til markedsføring',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'En type virus som infiserer datamaskiner automatisk',
      'Sosial manipulering for å stjele passord eller betalingsinformasjon',
      'En sikker krypteringsmetode for e-post',
      'Programvare som blokkerer spam',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Bokføringsloven krever at kvitteringsdetaljer beholdes i:',
    options: [
      '1 år',
      '3 år',
      '5 år',
      ...
      <div style={twoCol}>
        <div>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Mennesket er sikkerhetshullet
          </h2>
          <ul style={listStyle}>
            <li style={bodyText}>90 % av datainnbrudd skyldes menneskelige feil — ikke teknologisvikt.</li>
            <li style={bodyText}><Term word="GDPR" onShow={onShow} /> stiller strenge krav til bedrifters håndtering av persondata.</li>
            <li style={bodyText}><Term word="Informasjonssikkerhet" onShow={onShow} /> handler om rutiner like mye som teknologi.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1563013544-824ae1b704d3?auto=format&fit=crop&w=800&q=80" alt="Digital sikkerhet" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=800&q=80" alt="GDPR" />
        <div>
          <SlideLabel>Personvern</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="GDPR" onShow={onShow} /> — rettigheter og plikter
          </h2>
          <ul style={listStyle}>
            <li style={bodyText}>Data skal kun samles inn med klart <Term word="Behandlingsgrunnlag" onShow={onShow} />.</li>
            <li style={bodyText}><Term word="Dataminimering" onShow={onShow} />: samle kun det som er nødvendig.</li>
            <li style={bodyText}>Individet har rett til innsyn, retting og sletting av egne data.</li>
            <li style={{ ...bodyText, color: '#38bdf8', fontWeight: 600 }}>Brudd kan gi bøter opp til 4 % av global omsetning.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide3({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Trusler</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Phishing" onShow={onShow} /> — slik gjenkjenner du det
          </h2>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>Mistenkelig avsenderadresse (ikke offisiell domene)</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>Hastverk og trusler («kontoen din stenges om 24t»)</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>Lenker som ikke stemmer med teksten</li>
              <li style={{ ...bodyText, color: '#ef4444', fontWeight: 700 }}>Aldri klikk på lenker du er usikker på!</li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&w=800&q=80" alt="Phishing" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1614064641938-3bbee52942c7?auto=format&fit=crop&w=800&q=80" alt="Passord" />
        <div>
          <SlideLabel>Passordsikkerhet</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Grunnmuren i <Term word="Informasjonssikkerhet" onShow={onShow} />
          </h2>
          <ul style={listStyle}>
            <li style={bodyText}>Unike passord per tjeneste — aldri gjenbruk.</li>
            <li style={bodyText}>To-faktor autentisering (2FA) på alt som er viktig.</li>
            <li style={bodyText}>Lås PC-skjermen alltid når du forlater den (Windows + L).</li>
            <li style={{ ...bodyText, color: '#38bdf8', fontWeight: 600 }}>Passordbehandler anbefales for bedrifter.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Sletting</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Sletterutine" onShow={onShow} /> — rydding er sikkerhet
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Persondata skal slettes når formålet er oppfylt — men ikke alltid umiddelbart.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>Vitnemål: beholdes 5 år etter ansettelse</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>Kvitteringer: Bokføringsloven krever 5 år</li>
              <li style={{ ...bodyText, color: '#38bdf8' }}>Søknadsbrev fra avviste kandidater: slettes etter prosessen</li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&w=800&q=80" alt="Sletting" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=800&q=80" alt="HR" />
        <div>
          <SlideLabel>Praktisk eksempel</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            HR-avdeling ved oppsigelse
          </h2>
          <div style={{ ...bodyText, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem', marginBottom: 16 }}>
            Når en ansatt slutter, kan ikke HR-avdelingen beholde <em>alle</em> personopplysninger evig. Vitnemål beholdes 5 år. Lønnstransaksjoner arkiveres etter regnskapsregler.
          </div>
          <p style={bodyText}>En selger som forlater PC-en uten å låse skjermen bryter <Term word="GDPR" onShow={onShow} /> og setter kundedata i fare.</p>
        </div>
      </div>
    </Glass>
  )
    ...
    <Glass>
      <div style={twoCol}>
        <div>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            GDPR forbyr ikke all datalagring
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Mange tror at «rett til å bli slettet» betyr at kunder kan kreve å bli slettet fra alle systemer umiddelbart.
          </p>
          <div style={{ background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: 16, padding: '1.5rem' }}>
            <p style={{ ...bodyText, color: '#fbbf24' }}>
              <strong>Realiteten:</strong> Bokføringsloven krever at kvitteringsdetaljer beholdes i 5 år. <Term word="Dataminimering" onShow={onShow} /> betyr å ikke samle <em>unødvendig</em> data — ikke å slette nødvendig data.
            </p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&w=800&q=80" alt="Data" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={twoCol}>
        ...
        <div>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Personopplysningsloven og <Term word="GDPR" onShow={onShow} />
          </h2>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Personopplysningsloven:</strong> Inkorporerer GDPR i norsk lov.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Datatilsynet:</strong> Norsk tilsynsmyndighet — håndhever og veileder.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Bokføringsloven:</strong> Regulerer hvilke transaksjonsdata som MÅ beholdes.</li>
            <li style={{ ...bodyText, color: '#38bdf8' }}>datatilsynet.no — ressurs for bedrifter.</li>
          </ul>
        </div>
      </div>
    </Glass>
    ...
    <Glass>
      <div style={twoCol}>
        <div>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Sikkerhet er en kultur, ikke en teknologi
          </h2>
          <ul style={listStyle}>
            <li style={bodyText}><Term word="GDPR" onShow={onShow} /> — behandle kun data du trenger, med lovlig grunnlag.</li>
            <li style={bodyText}><Term word="Phishing" onShow={onShow} /> — vær skeptisk, sjekk avsender, klikk aldri blindt.</li>
            <li style={bodyText}><Term word="Sletterutine" onShow={onShow} /> — rydd i datasystemene jevnlig.</li>
            <li style={{ ...bodyText, color: '#38bdf8', fontWeight: 600 }}>Lås skjermen. Alltid. Hver gang.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=800&q=80" alt="Sikkerhet" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Hvilke persondata samler bedriften du jobber i (eller drømmer om å jobbe i) — og er det egentlig nødvendig?
        </h2>
        <p style={{ fontSize: 18, color: '#cbd5e1', lineHeight: 1.7 }}>
          Tenk dataminimering: hva ville du slettet, og hva er du lovpålagt å beholde?
        </p>
      </div>
    </Glass>
  )
...
export default function DigitalSikkerhetPersonvernPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Digital sikkerhet og personvern',
    }))
    setLivePin(pin)
