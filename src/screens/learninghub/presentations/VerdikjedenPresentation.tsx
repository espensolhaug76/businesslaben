...
const TERMS: Record<string, string> = {
  'Verdikjede': 'Alle aktivitetene et produkt går gjennom fra råvare til ferdig vare i hendene på kunden.',
  'Primæraktiviteter': 'De kjerneaktivitetene som direkte skaper verdi: innkjøp, produksjon, salg og service.',
  'Støtteaktiviteter': 'Aktiviteter som hjelper primæraktivitetene å fungere, f.eks. IT, HR og ledelse.',
  'Bærekraft': 'Å møte dagens behov uten å ødelegge muligheten for fremtidige generasjoner til å møte sine behov.',
  'Sirkulær økonomi': 'Et økonomisk system der produkter og materialer gjenbrukes og gjenvinnes i stedet for å kastes.',
  'Kvalitetshjulet': 'En forbedringssyklus: Planlegg → Gjør → Kontroller → Korriger (PDCA-syklusen).',
  'High Yield – Low Impact': 'Strategi der man tiltrekker seg turister som bruker mye penger men etterlater seg lite skade på naturen.',
  'Sosial bærekraft': 'Å sikre rettferdige arbeidsforhold og menneskerettigheter gjennom hele verdikjeden.',
}

const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva er primæraktiviteter i verdikjeden?',
    options: [
      'Aktiviteter som støtter og muliggjør andre aktiviteter',
      'Aktiviteter som direkte bidrar til å skape og levere produktet til kunden',
      'Administrative og finansielle aktiviteter i bedriften',
      'HR-aktiviteter som rekruttering og opplæring',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'En økonomi basert på økt forbruk og produksjon',
      'En økonomi der produkter og materialer gjenbrukes og gjenvinnes lengst mulig',
      'En global frihandelsøkonomi uten toll og avgifter',
      'En økonomi drevet av fossil energi og lineær produksjon',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva står PDCA for i kvalitetshjulet?',
    options: [
      'Produce, Distribute, Control, Advertise',
      'Plan, Do, Check, Act',
      'Price, Design, Create, Analyze',
      'Plan, Develop, Complete, Assess',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er støtteaktiviteter i verdikjeden?',
    options: [
      'Kjerneaktiviteter som direkte skaper produktet',
      'Aktiviteter som hjelper primæraktivitetene å fungere, som IT, HR og økonomi',
      'Kundeserviceaktiviteter etter salget',
      'Markedsføringsaktiviteter for å nå nye kunder',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er High Yield – Low Impact-strategien?',
    options: [
      'Å produsere så mange varer som mulig med minst mulig ressurser',
      'Å tiltrekke turister som bruker mye penger men etterlater seg lite skade på naturen',
      'Å maksimere utbytte til aksjonærene med minimale investeringer',
      'Å markedsføre produkter med høy margin til så få kunder som mulig',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Verdikjeden og<br />
        <span style={{ color: '#38bdf8', fontStyle: 'italic' }}>bærekraft</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>
        Fra råvare til ferdig produkt – og ansvaret vi har for miljø og mennesker på veien.
      </p>
      {_isLive ? (
...
function Slide1({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 3 – Verdikjeden</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Hva er en <Term word="Verdikjede" onShow={onShow} />?
          </h2>
          <p style={bodyText}>
            Det er veien et produkt går fra det bare er en råvare (som kaffebønner i Brasil) til det serveres som en kaffe på Starbucks. Hvert stopp på veien skal gjøre produktet mer verdifullt.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&w=800&q=80" alt="Kaffe verdikjede" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1553413077-190dd305871c?auto=format&fit=crop&w=800&q=80" alt="Produksjon" />
        <div>
          <SlideLabel>Verdiskapning</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Primæraktiviteter" onShow={onShow} /> – Selve jobben
          </h2>
          <p style={bodyText}>
            Det handler om å få varene inn på lager, lage produktet, selge det og gi service etterpå. Dette er hjertet i bedriften.
          </p>
        </div>
      </div>
    </Glass>
...
function Slide3({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Støttefunksjoner</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Støtteaktiviteter" onShow={onShow} /> – Motoren bak
          </h2>
          <p style={bodyText}>
            Ting som IT-systemer, gode ledere og innkjøpsavtaler som gjør at primæraktivitetene går som smurt.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1551434678-e076c223a692?auto=format&fit=crop&w=800&q=80" alt="IT og støtte" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1497435334941-8c899ee9e8e9?auto=format&fit=crop&w=800&q=80" alt="Miljø" />
        <div>
          <SlideLabel>Miljø og bærekraft</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Miljøvennlig verdi
          </h2>
          <p style={bodyText}>
            Vi må sjekke om transporten forurenser, og om vi bruker for mye plast. <Term word="Bærekraft" onShow={onShow} /> betyr at vi ikke ødelegger naturen for de som kommer etter oss.
          </p>
        </div>
      </div>
    </Glass>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Mennesker i kjeden</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Sosial bærekraft" onShow={onShow} />
          </h2>
          <p style={bodyText}>
            Det handler om menneskene i kjeden. Får kaffebonden i Brasil en rettferdig lønn? Er fabrikken trygg?
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1532629345422-7515f3d16bb6?auto=format&fit=crop&w=800&q=80" alt="Sosial bærekraft" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1611284446314-60a58ac0debb?auto=format&fit=crop&w=800&q=80" alt="Sirkulær økonomi" />
        <div>
          <SlideLabel>Fremtidens økonomi</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Sirkulær økonomi" onShow={onShow} /> – Slutt på søppel
          </h2>
          <p style={bodyText}>
            I stedet for å kaste ting, lager vi dem slik at de kan repareres eller gjenvinnes til noe nytt. Målet er en sirkel der ingenting går til spille.
          </p>
        </div>
      </div>
    </Glass>
...
function Slide7({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Kontinuerlig forbedring</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Kvalitetshjulet" onShow={onShow} />
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>En metode for å aldri slutte å forbedre seg:</p>
          <ul style={listStyle}>
            <li style={bodyText}><span style={{ color: '#38bdf8', fontWeight: 700 }}>1. Planlegg</span> – Hva skal vi fikse?</li>
            <li style={bodyText}><span style={{ color: '#38bdf8', fontWeight: 700 }}>2. Gjør</span> – Prøv det ut</li>
            <li style={bodyText}><span style={{ color: '#38bdf8', fontWeight: 700 }}>3. Kontroller</span> – Virka det?</li>
            <li style={bodyText}><span style={{ color: '#38bdf8', fontWeight: 700 }}>4. Korriger</span> – Gjør det permanent eller prøv noe annet</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=800&q=80" alt="Kvalitet" />
      </div>
    </Glass>
  )
}
...
function Slide8({ onShow: _onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1555041469-a586c61ea9bc?auto=format&fit=crop&w=800&q=80" alt="IKEA møbler" />
        <div>
          ...
          <div style={{ ...bodyText, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem', marginBottom: 16 }}>
            De har begynt å kjøpe tilbake dine gamle IKEA-møbler. De selger dem på nytt i "gjenbrukshjørnet". Det er bra for miljøet og lommeboka!
          </div>
        </div>
      </div>
    </Glass>
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Analyse – Norsk reiseliv</SlideLabel>
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          <Term word="High Yield – Low Impact" onShow={onShow} />
        </h2>
        <p style={{ ...bodyText, fontSize: 20 }}>
          I norsk reiseliv vil vi ha færre turister som legger igjen mer penger, slik at vi ikke sliter ut naturen vår.
        </p>
      </div>
    </Glass>
  )
...
export default function VerdikjedenPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Verdikjeden og bærekraft',
    }))
    setLivePin(pin)
