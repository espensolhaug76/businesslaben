...
const TERMS: Record<string, string> = {
  'Profesjonell etikk': 'Å handle i samsvar med yrkesetiske normer – ærlighet, respekt og integritet i alle situasjoner.',
  'Etisk dilemma': 'En situasjon der man må velge mellom to motstridende verdier eller riktige handlinger.',
  'Sosial bærekraft': 'Å skape inkluderende og rettferdige arbeidsplasser og samfunn der alle har det bra.',
  'CSR': 'Corporate Social Responsibility – bedriftens frivillige ansvar overfor samfunnet utover lovpålagte krav.',
  'Grønnvasking': 'Å gi et misvisende bilde av at produkter eller bedriften er mer miljøvennlig enn det faktisk er.',
  'Inkluderende rekruttering': 'Ansettelsespraksis som aktivt søker å inkludere grupper som ofte faller utenfor arbeidsmarkedet.',
}
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva er etikk?',
    options: [
      'En type regnskapsstandard for bærekraftig rapportering',
      'Prinsipper og normer for riktig og god atferd',
      'En markedsføringsstrategi for ansvarlige bedrifter',
      'Et juridisk krav fra myndighetene',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Rask vekst som maksimerer profitt på kort sikt',
      'Utvikling som dekker dagens behov uten å ødelegge fremtidige generasjoners muligheter',
      'Utelukkende utvikling av bærekraftige materialer og produkter',
      'Nedskjæring i kostnader for å øke lønnsomheten',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva betyr CSR (Corporate Social Responsibility)?',
    options: [
      'En type teknisk standard for datasystemer',
      'Bedriftens samfunnsansvar og forpliktelse til ansvarlig forretningsdrift',
      'En metode for markedsundersøkelse og kundeanalyse',
      'En finansieringsform for oppstartsbedrifter',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'En bærekraftig vaskeprosess som bruker mindre vann',
      'Å gi et misvisende bilde av at produkter eller bedriften er mer miljøvennlig enn de er',
      'En sertifiseringsordning for miljøvennlige bedrifter',
      'En type markedsføring rettet mot miljøbevisste forbrukere',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'En situasjon der man bryter loven for å oppnå et mål',
      'En situasjon der man må velge mellom to motstridende, men likeverdig riktige verdier',
      'Et spørsmål om hva som er lønnsomt for bedriften',
      'En uenighet mellom ansatte om arbeidsrutiner',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Etikk og<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>bærekraft</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 20 – Profesjonell etikk og bærekraft i samhandling.</p>
      {_isLive ? (
...
function Slide1({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 20 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Profesjonell etikk" onShow={show} /></h2>
          <p style={bodyText}>Det betyr å gjøre det som er rett, selv om ingen ser på. Det handler om å være ærlig mot kunden og behandle alle med respekt.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Ærlighet mot kunden', 'Respekt for alle', 'Integritet i alle situasjoner'].map(item => (
        ...
        <SlideImg src="https://images.unsplash.com/photo-1521791136064-7986c2920216?auto=format&fit=crop&w=800&q=80" alt="Profesjonell etikk" />
      </div>
    </Glass>
  )
}
...
function Slide2({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=800&q=80" alt="Etiske dilemmaer" />
        <div>
          <SlideLabel>Modul 20 – Slide 2</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Etisk dilemma" onShow={show} /></h2>
          <p style={bodyText}>Situasjoner der to verdier kræsjer.</p>
          <div style={{ marginTop: 24, padding: '1.5rem', background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.25)', borderRadius: 16 }}>
            <p style={{ color: '#fbbf24', fontWeight: 600, fontSize: 16, margin: 0, fontStyle: 'italic' }}>Eks: Skal du selge en kjempedyr forsikring til en gammel dame, selv om du vet hun ikke trenger den?</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 20 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Sosial bærekraft" onShow={show} /></h2>
          <p style={bodyText}>Å skape en arbeidsplass der alle føler seg trygge og inkludert, uansett hvor de kommer fra eller hvem de er.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Trygt arbeidsmiljø for alle', 'Inkludering uavhengig av bakgrunn', 'Likeverd og rettferdighet'].map(item => (
              <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#cbd5e1', fontSize: 16 }}>
                <span style={{ color: '#38bdf8', fontWeight: 800 }}>→</span> {item}
              </li>
            ))}
        ...
        <SlideImg src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=800&q=80" alt="Sosial bærekraft" />
      </div>
    </Glass>
  )
}
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1508193638397-1c4234db14d8?auto=format&fit=crop&w=800&q=80" alt="CSR" />
        <div>
          <SlideLabel>Modul 20 – Slide 4</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Bedriftens samfunnsansvar (<Term word="CSR" onShow={show} />)</h2>
          <p style={bodyText}>At bedriften gjør noe bra for lokalmiljøet, for eksempel ved å støtte det lokale idrettslaget.</p>
        </div>
      </div>
    </Glass>
  )
...
function Slide5({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 20 – Slide 5</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Grønnvasking" onShow={show} /> – Miljø-juks</h2>
          <p style={bodyText}>Når en bedrift bruker masse penger på å si at de er miljøvennlige, men egentlig forurenser de like mye som før. Dette er uetisk og ulovlig.</p>
          <div style={{ marginTop: 20, padding: '1.25rem 1.5rem', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 14 }}>
            <p style={{ color: '#f87171', fontWeight: 600, fontSize: 15, margin: 0 }}>Uetisk og ulovlig – unngå dette!</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Grønnvasking" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 20 – Slide 6 – Case</SlideLabel>
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Case: Stormberg</h2>
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 24, padding: '2.5rem' }}>
          <p style={{ ...bodyText, fontSize: 18, margin: 0 }}>De er kjent for <Term word="Inkluderende rekruttering" onShow={show} />. De ansetter folk som har slitt tidligere (f.eks. eks-kriminelle). Dette er <Term word="Sosial bærekraft" onShow={show} /> i praksis.</p>
        </div>
        <p style={{ color: '#64748b', fontSize: 15, marginTop: 20, fontStyle: 'italic' }}>Resultat: Et sterkere samfunn og en mer mangfoldig arbeidsstyrke.</p>
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Kan en bedrift være "for snill" og gå konkurs av det?</h2>
        <p style={{ ...bodyText, fontSize: 20 }}>Diskuter balansen mellom samfunnsansvar og lønnsomhet – når er etikk og bærekraft en fordel, og når kan det bli for kostbart?</p>
      </div>
    </Glass>
  )
}

export default function EtikkBaerekraftPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Etikk og bærekraft',
    }))
    setLivePin(pin)
