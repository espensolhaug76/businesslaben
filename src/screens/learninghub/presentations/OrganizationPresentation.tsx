...
const TERMS: Record<string, string> = {
  'Hierarkisk struktur': 'Organisering der sjefen på toppen bestemmer og beskjeder sendes nedover i kjeden. Vanlig i store selskaper og Forsvaret.',
  'Flat struktur': 'Organisering med få ledernivåer og kort vei til toppen. Gir ansatte mer myndighet til å ta beslutninger.',
  'Empowerment': 'Å gi ansatte makt og myndighet til å ta egne valg og løse kundeproblemer uten å spørre sjefen.',
  'Linje': 'Ansatte som jobber direkte med det bedriften lever av, f.eks. selgere eller hotellresepsjonister.',
  'Stab': 'Støttefunksjoner som hjelper linjeansatte, f.eks. IT, HR og økonomiavdelingen.',
  'Organisasjonskart': 'En visuell fremstilling av bedriftens struktur som viser hvem som rapporterer til hvem.',
  'Kommandolinje': 'Den formelle rapporteringsveien fra topp til bunn i en hierarkisk organisasjon.',
}

const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva kjennetegner en hierarkisk organisasjonsstruktur?',
    options: [
      'Alle ansatte er likestilte og tar beslutninger sammen',
      'Pyramideformet med klare ansvarslinjer og kommandokjede',
      'Ingen formelle lederstillinger i organisasjonen',
      'Flat struktur med høy grad av autonomi',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva betyr empowerment i organisasjonssammenheng?',
    options: [
      'Sentralisert beslutningstaking hos toppledelsen',
      'Å gi ansatte myndighet og ansvar til å ta beslutninger selv',
      'Økt kontroll og overvåking av ansatte',
      'Standardisering av alle arbeidsrutiner',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er en stabsfunksjon i en organisasjon?',
    options: [
      'En direkte produksjonslinje med ansvar for leveranse',
      'En støttefunksjon som gir råd og service til linjeledelsen',
      'En kundekontaktfunksjon i frontlinjen',
      'En salgsavdeling med eget budsjettansvar',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er en flat organisasjonsstruktur?',
    options: [
      'En hierarkisk struktur med mange ledernivåer',
      'En struktur med få ledernivåer der ansatte har stor grad av selvstendighet',
      'En matrise-organisasjon med dobbel rapportering',
      'En nettverksorganisasjon uten formelle ledere',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er kommandolinjen i en organisasjon?',
    options: [
      'IT-kommandoer for å styre bedriftens datasystemer',
      'Den formelle rapporteringsveien fra toppledelse til laveste nivå',
      'Kommunikasjonslinjen mellom bedriften og kundene',
      'En spesialbestilling av varer fra leverandørene',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Roller og<br />
        <span style={{ color: '#38bdf8', fontStyle: 'italic' }}>organisasjon</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>
        Hvem gjør hva? Ansvarsfordeling, roller og organisasjonskart i servicenæringen.
      </p>
...
function Slide1({ onShow: _onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 2 – Organisasjon</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Hvem gjør hva?
          </h2>
          <p style={bodyText}>
            En bedrift trenger en plan for hvem som skal rydde, hvem som tar kassa, og hvem som snakker med media. Uten tydelig ansvar oppstår det kaos og krangling.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&w=800&q=80" alt="Team møte" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=800&q=80" alt="Hierarki kontor" />
        <div>
          <SlideLabel>Organisasjonsstruktur</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Hierarkisk struktur" onShow={onShow} /> – Pyramiden
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Her sitter den øverste sjefen på toppen og bestemmer alt. Beskjedene sendes nedover i <Term word="Kommandolinje" onShow={onShow} />r.
          </p>
          <p style={bodyText}>
            Dette er vanlig i Forsvaret eller store selskaper som Equinor.
          </p>
        </div>
      </div>
    </Glass>
    ...
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Organisasjonsstruktur</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Flat struktur" onShow={onShow} /> – Alle blir hørt
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Her er det få sjefer og kort vei til toppen. De ansatte får <Term word="Empowerment" onShow={onShow} />, som betyr makt til å ta egne valg for å hjelpe kunden der og da.
          </p>
          <p style={bodyText}>
            Eksempel: En liten lokal kafé.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&w=800&q=80" alt="Flat organisasjon" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=800&q=80" alt="Linje og stab" />
        <div>
          <SlideLabel>Roller i bedriften</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Linje" onShow={onShow} /> og <Term word="Stab" onShow={onShow} />
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}>
              <strong style={{ color: '#f1f5f9' }}>Linje:</strong> De som jobber med det bedriften lever av (f.eks. selgere i en butikk).
            </li>
            <li style={bodyText}>
              <strong style={{ color: '#f1f5f9' }}>Stab:</strong> De som støtter dem (f.eks. IT-folk eller de som regner ut lønn).
            </li>
          </ul>
        </div>
      </div>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Visualisering av struktur</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Organisasjonskart" onShow={onShow} />
          </h2>
          <p style={bodyText}>
            En tegning av hvordan bedriften er bygd opp. Det er som et kart over hvem du skal spørre hvis du lurer på noe.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&w=800&q=80" alt="Organisasjonskart" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow: _onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=800&q=80" alt="Hotell" />
        <div>
          ...
          <div style={{ ...bodyText, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem', marginBottom: 16 }}>
            Hotellene deres har ofte en flat struktur. Resepsjonisten har lov til å gi en gratis brus til en misfornøyd gjest uten å ringe sjefen først. Det kalles å ta ansvar!
          </div>
        </div>
      </div>
    </Glass>
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Hvorfor er det ofte raskere å få god service i en bedrift med flat struktur?
        </h2>
        <p style={{ ...bodyText, fontSize: 20 }}>
          Tenk på hva som skjer når en ansatt kan ta egne valg versus når de må spørre sjefen om lov til alt.
        </p>
      </div>
    </Glass>
  )
...
export default function OrganizationPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Roller og organisasjon',
    }))
    setLivePin(pin)
