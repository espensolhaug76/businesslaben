...
const TERMS: Record<string, string> = {
  'CRM': 'Customer Relationship Management — system som samler all kundeinformasjon, kjøpshistorikk og interaksjon på ett sted for helhetlig kundeoversikt.',
  'Salgstrakt': 'Visualisering av salgsprosessen fra mange leads øverst til færre kjøpende kunder nederst. Hjelper bedriften å prioritere og predikere inntekter.',
  'Lead': 'En person eller virksomhet som har vist interesse — lastet ned en ressurs, meldt seg på nyhetsbrev eller fylt ut et skjema.',
  'Konverteringsrate': 'Andelen leads som faktisk blir betalende kunder. Lav rate indikerer problemer med kvalifisering eller tilbud.',
  'Ticketing': 'Systemet som gir kundeklager og serviceforespørsler et saksnummer og sporer dem fra opprettelse til løsning.',
  'SLA': 'Service Level Agreement — definerte svartider for kundeservice. Systemet varsler når SLA er i ferd med å brytes.',
  'Churn': 'Frafall av kunder — når kunder slutter å bruke tjenesten eller sier opp abonnementet.',
  'GDPR': 'Personvernforordningen som setter krav til lovlig behandling, lagringstid og tilgangskontroll av personopplysninger i CRM.',
  'Lead-scoring': 'AI-basert rangering av leads basert på adferd (sidebesøk, e-post-åpning) for å hjelpe selgere å prioritere de mest lovende.',
  'SuperOffice': 'Norsk-utviklet CRM-system brukt av tusenvis av norske B2B-bedrifter. Integrerer salg, markedsføring og kundeservice.',
}
...
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    ...
    options: [
      'Det lagrer bilder av kundene',
      'Det samler all interaksjon, kjøpshistorikk og preferanser for en helhetlig kundeoversikt',
      'Det sender automatisk julekort til kundene',
      'Det er raskere enn Excel',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'En bedrift har 500 leads men bare 2 konverterer til kunder. Hva indikerer dette?',
    options: [
      'At bedriften trenger mer markedsføring for å generere flere leads',
      'At noe er galt i kvalifiseringsprosessen — leads er for dårlig kvalifiserte',
      'At prisen er for høy',
      'At CRM-systemet fungerer dårlig',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'En CRM-database inneholder kontakter fra en messe for 5 år siden. Ingen ble kunder. Hva bør gjøres etter GDPR?',
    options: [
      'Beholdes — de kan bli kunder i fremtiden',
      'Slettes — formålet med lagringen er ikke lenger aktuelt',
      'Overføres til en passiv database for å spare plass',
      'Sendes masse-epost for å sjekke om noen er interessert',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Digitale system og<br />
        <span style={{ color: '#38bdf8', fontStyle: 'italic' }}>kundeoppfølging</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>
        CRM, salgstrakt og GDPR i servicebransjen
      </p>
...
function Slide1({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Fase 1 – CRM</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="CRM" onShow={onShow} /> — fra adressebok til sentralnervesystem
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#f87171' }}>Uten CRM:</strong> Kunden ringer og forklarer problemet sitt for tredje gang til en ny person.</li>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#4ade80' }}>Med CRM:</strong> All dialog, kjøp og preferanser er logget. Alle ansatte ser hele historikken umiddelbart.</li>
            <li style={bodyText}>Over 90 % av Fortune 500-bedrifter bruker CRM.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="CRM system" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow: _onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80" alt="CRM arkitektur" />
        <div>
          <SlideLabel>Fase 2 – CRM-arkitektur</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Datadisiplin er nøkkelen
          </h2>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#f1f5f9', marginBottom: 6 }}>Kontaktobjekter — individer og virksomheter</li>
              <li style={{ ...bodyText, color: '#f1f5f9', marginBottom: 6 }}>Aktivitetslogg — alle møter og samtaler</li>
              <li style={{ ...bodyText, color: '#f1f5f9', marginBottom: 6 }}>Muligheter (Deals) — salgsprognoser</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>Saker (Cases) — kundeklager og service</li>
            </ul>
          </div>
        </div>
      </div>
...
function Slide3({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Fase 3 – Salgstrakt</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Lead" onShow={onShow} /> til kunde — <Term word="Salgstrakt" onShow={onShow} />
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#f1f5f9' }}>Lead:</strong> Viser interesse (nyhetsbrev, skjema)</li>
            <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#f1f5f9' }}>Prospekt:</strong> Kvalifisert — budsjett og behov</li>
            <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#f1f5f9' }}>Mulighet:</strong> Tilbud sendt, forhandling pågår</li>
            <li style={{ ...bodyText }}><strong style={{ color: '#38bdf8' }}><Term word="Konverteringsrate" onShow={onShow} />:</strong> 100 leads → 20 → 5 → 2 salg</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=800&q=80" alt="Salgstrakt" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="Ticketing kundeservice" />
        <div>
          <SlideLabel>Fase 4 – Ticketing</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Ticketing" onShow={onShow} /> — kundeservice som saksbehandling
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}>Hver henvendelse får et saksnummer — kunden kan følge status.</li>
            <li style={{ ...bodyText, marginBottom: 12 }}><Term word="SLA" onShow={onShow} />: definerte svartider — systemet varsler om SLA brytes.</li>
            <li style={bodyText}>Populære systemer: Zendesk, Freshdesk og HubSpot Service Hub.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Fase 5 – AI og automasjon</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Fremtiden er allerede her
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#f1f5f9' }}><Term word="Lead-scoring" onShow={onShow} />:</strong> AI analyserer adferd og gir leads en score. Høy score = prioriter nå.</li>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#f1f5f9' }}>Churn-prediksjon:</strong> AI identifiserer kunder i ferd med å forlate — slik at selgere kan handle proaktivt.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>E-post-automasjon:</strong> Trigger-baserte sekvenser frigjør tid til viktig menneskelig kontakt.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80" alt="AI automasjon" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?auto=format&fit=crop&w=800&q=80" alt="SuperOffice HubSpot" />
        <div>
          <SlideLabel>Case – SuperOffice og HubSpot</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            CRM i norsk kontekst
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#38bdf8' }}><Term word="SuperOffice" onShow={onShow} />:</strong> Norsk-utviklet, brukt av tusenvis av B2B-bedrifter. Integrerer salg, markedsføring og service.</li>
            <li style={bodyText}><strong style={{ color: '#94a3b8' }}>HubSpot:</strong> Global markedsleder. Starter gratis, skalerer med bedriften. Sterk på inbound marketing.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide7({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Case – Komplett.no og Zendesk</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Kundeservice i stor skala
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Komplett.no behandler titusener av ordrer ukentlig. Uten strukturert <Term word="Ticketing" onShow={onShow} /> er kundeservice kaotisk.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1rem 1.5rem' }}>
            <p style={{ ...bodyText, color: '#38bdf8', fontWeight: 700, textAlign: 'center' }}>Saksnummeret er kundens garanti for at de ikke er glemt</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=800&q=80" alt="Netthandel kundeservice" />
      </div>
    </Glass>
  )
}
...
function Slide8({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="CRM misforståelse" />
        <div>
          <SlideLabel>Fase 8 – Misforståelse</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="CRM" onShow={onShow} /> er ikke en adressebok
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#f87171' }}>Myten:</strong> "CRM er bare en glorifisert digital adressebok."</li>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#4ade80' }}>Realiteten:</strong> CRM predikerer inntekt, identifiserer <Term word="Churn" onShow={onShow} />-risiko og automatiserer kommunikasjon.</li>
            <li style={bodyText}>CRM-verdi = kvaliteten på dataregistreringen. Garbage in, garbage out.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide9({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Fase 9 – GDPR og CRM</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Lagring med ansvar — <Term word="GDPR" onShow={onShow} />
          </h2>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#f1f5f9', marginBottom: 8 }}><strong>Behandlingsgrunnlag:</strong> Samtykke, kontrakt eller berettiget interesse</li>
              <li style={{ ...bodyText, color: '#f1f5f9', marginBottom: 8 }}><strong>Lagringstid:</strong> Slett data når formålet er oppfylt</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}><strong>Tilgangskontroll:</strong> Kun de som trenger data for jobben skal ha tilgang</li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?auto=format&fit=crop&w=800&q=80" alt="GDPR personvern" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Hva skjer med bedriften din om alle selgere slutter i morgen — og ingen har registrert noe i CRM?
        </h2>
        <p style={{ ...bodyText, fontSize: 20 }}>
          Den bedriften som kjenner kundene sine best, vinner. CRM er infrastrukturen for den kunnskapen.
        </p>
      </div>
    </Glass>
  )
...
export default function DigitaleSystemKundeoppfolgingPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Digitale system og kundeoppfølging',
    }))
    setLivePin(pin)
