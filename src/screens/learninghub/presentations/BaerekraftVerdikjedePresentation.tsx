...
const TERMS: Record<string, string> = {
  'Verdikjede': 'Alle aktiviteter og ledd som bidrar til å skape og levere et produkt, fra råvare til sluttbruker.',
  'Bærekraft': 'Å drive virksomhet på en måte som ivaretar miljø, sosiale forhold og økonomi for nåværende og fremtidige generasjoner.',
  'Scope 3': 'Indirekte klimagassutslipp i hele verdikjeden — leverandørkjede, transport, ansattes reiser og produktbruk. Utgjør typisk over 90 % av en bedrifts totale fotavtrykk.',
  'Åpenhetsloven': 'Norsk lov (2022) som pålegger større virksomheter å gjennomføre aktsomhetsvurderinger og offentliggjøre informasjon om leverandørkjedens arbeidsforhold.',
  'Sporbarhet': 'Evnen til å følge et produkt tilbake gjennom alle ledd i produksjonen — dokumentasjon av hele råvare-til-hylle-reisen.',
  'Grønnvasking': 'Å markedsføre seg som miljøvennlig uten å adressere de faktiske største utslippskildene eller uten å ha reelle bærekraftstiltak.',
  'Modal shift': 'Overflytting av gods fra ett transportmiddel (bil) til et mer klimavennlig alternativ (jernbane eller skip).',
  'Sirkulær økonomi': 'Produksjonssystem der ressurser holdes i omløp lengst mulig, i motsetning til den lineære "ta-bruke-kaste"-modellen.',
  'Aktsomhetsvurdering': 'Risikobasert undersøkelse for å identifisere og håndtere faktisk og potensiell skade på menneskerettigheter i leverandørkjeden.',
  'ESG': 'Environmental, Social and Governance — rammeverk for å vurdere en bedrifts bærekraft og samfunnsansvar.',
}

const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva kjennetegner Scope 3-utslipp?',
    options: [
      'Indirekte utslipp i leverandørkjeden som typisk utgjør over 90 % av fotavtrykket',
      'Direkte utslipp fra bedriftens egne kjøretøy og fyringsolje',
      'Utslipp fra kjøpt strøm og fjernvarme',
      'Utslipp fra bedriftens kontorbygg',
    ],
    correct: 0,
    timeSeconds: 45,
  },
  {
    question: 'Hvem gjelder Åpenhetsloven for?',
    options: [
      'Alle norske bedrifter uavhengig av størrelse',
      'Kun børsnoterte selskaper med over 500 ansatte',
      'Virksomheter med minst 50 ansatte, 70 mill. kr i omsetning og 35 mill. kr i balanse',
      'Bare bedrifter som importerer varer fra utlandet',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  {
    question: 'Hva er "modal shift" i transportsammenheng?',
    options: [
      'At transportselskapet skifter til en ny forretningsmodell',
      'Overflytting av gods fra bil til mer klimavennlig transportform som jernbane eller skip',
      'En ny metode for å pakke gods mer effektivt',
      'Digitalisering av fraktdokumentasjon',
    ],
    correct: 1,
    timeSeconds: 45,
  },
...
function Slide1({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Fase 1 – Bærekraftsansvar</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Den bærekraftige kjeden
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}>
              <strong style={{ color: '#f1f5f9' }}>Utvidet ansvar:</strong> Bedriften er ansvarlig for mer enn egne aktiviteter — inkludert leverandørers praksis.
            </li>
            <li style={{ ...bodyText, marginBottom: 12 }}>
              Tre dimensjoner: Miljø, Sosialt og Økonomi — alle tre må ivaretas for ekte <Term word="Bærekraft" onShow={onShow} />.
            </li>
            <li style={bodyText}>
              Fra lineær til <Term word="Sirkulær økonomi" onShow={onShow} />: ressurser holdes i omløp fremfor å kastes.
            </li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1441974231531-c6227db76b6e?auto=format&fit=crop&w=800&q=80" alt="Bærekraft natur" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1532601224476-15c79f2f7a51?auto=format&fit=crop&w=800&q=80" alt="Klimautslipp" />
        <div>
          <SlideLabel>Fase 2 – Klimafotavtrykk</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Scope 3" onShow={onShow} /> — det usynlige problemet
          </h2>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#f1f5f9' }}><strong>Scope 1:</strong> Egne direkte utslipp</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}><strong>Scope 2:</strong> Utslipp fra kjøpt energi</li>
              <li style={{ ...bodyText, color: '#38bdf8', fontWeight: 700 }}><strong>Scope 3:</strong> Alle indirekte utslipp i verdikjeden — typisk 90 %+</li>
            </ul>
          </div>
        </div>
      </div>
...
function Slide3({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Fase 3 – Porters verdikjede</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Verdikjede" onShow={onShow} /> — verdi i hvert ledd
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#f1f5f9' }}>1. Inngående logistikk</strong> — mottak og lagring av innsatsvarer</li>
            <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#f1f5f9' }}>2. Drift</strong> — selve produksjonen eller tjenesteleveransen</li>
            <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#f1f5f9' }}>3. Utgående logistikk</strong> — levering til kunder</li>
            <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#f1f5f9' }}>4. Markedsføring og salg</strong></li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>5. Service</strong> — retur og resirkulering</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="Verdikjede logistikk" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80" alt="Sporbarhet teknologi" />
        <div>
          <SlideLabel>Fase 4 – Sporbarhet</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Sporbarhet" onShow={onShow} /> — vite hvor varen kommer fra
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}>Blockchain og RFID muliggjør nær-sanntids sporing i komplekse verdikjeder.</li>
            <li style={{ ...bodyText, marginBottom: 12 }}><Term word="Åpenhetsloven" onShow={onShow} /> krever at norske bedrifter offentliggjør leverandørinformasjon.</li>
            <li style={bodyText}>Stadig flere forbrukere ønsker å vite hvem som produserte varene og under hvilke forhold.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Fase 5 – Leverandøroppfølging</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Stille krav og sjekke
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#f1f5f9' }}>Kravspesifikasjon:</strong> Klare krav til miljø, arbeidsforhold og antikorrupsjon i kontrakter.</li>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#f1f5f9' }}>Revisjoner:</strong> Tredjepartsrevisjoner som SA8000 og uanmeldte besøk.</li>
            <li style={bodyText}><Term word="Aktsomhetsvurdering" onShow={onShow} />: risikobasert undersøkelse for å avdekke brudd på menneskerettigheter.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1589829545856-d10d557cf95f?auto=format&fit=crop&w=800&q=80" alt="Leverandørkontroll" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1532601224476-15c79f2f7a51?auto=format&fit=crop&w=800&q=80" alt="Tog gods jernbane" />
        <div>
          <SlideLabel>Case – CargoNet</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Modal shift" onShow={onShow} /> fra vei til jernbane
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            CargoNet erstatter i snitt 1 200 trailere per dag ved å kjøre gods på jernbane — og sparer nesten 700 000 CO₂-ekvivalenter per år.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1rem 1.5rem' }}>
            <p style={{ ...bodyText, color: '#38bdf8', fontWeight: 700, textAlign: 'center' }}>Jernbane er 7–10× mer energieffektivt enn lastebil</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide7({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Case – Voice</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Åpenhet som strategi
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Voice (Match og VIC) offentliggjør lister over sine produksjonsfabrikker, slik at journalister og NGO-er kan gjennomføre uavhengige inspeksjoner.
          </p>
          <p style={bodyText}>
            <Term word="Grønnvasking" onShow={onShow} />: å markedsføre seg som grønn uten å adressere de faktiske største utslippskildene.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1450101499163-c8848c66ca85?auto=format&fit=crop&w=800&q=80" alt="Tekstilbransjen åpenhet" />
      </div>
    </Glass>
  )
}
...
function Slide8({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1441974231531-c6227db76b6e?auto=format&fit=crop&w=800&q=80" alt="Bærekraftsmisforståelser" />
        <div>
          <SlideLabel>Fase 8 – Misforståelser</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Bærekraft er mer enn kildesortering
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#38bdf8' }}>Myte:</strong> Bærekraft = kildesortering og papirstrå.</li>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#38bdf8' }}>Myte:</strong> Bærekraft er dyrt. Forskning viser at bærekraftige bedrifter er mer lønnsomme over tid.</li>
            <li style={bodyText}><strong style={{ color: '#38bdf8' }}>Myte:</strong> Bare store bedrifter har ansvar. <Term word="Åpenhetsloven" onShow={onShow} /> gjelder fra 50 ansatte.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide9({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Fase 9 – Lovverk</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Åpenhetsloven" onShow={onShow} /> (2022)
          </h2>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#f1f5f9', marginBottom: 8 }}><strong>Hvem:</strong> 50+ ansatte, 70 mill. kr omsetning, 35 mill. kr balanse</li>
              <li style={{ ...bodyText, color: '#f1f5f9', marginBottom: 8 }}><strong>Krav:</strong> Gjennomfør <Term word="Aktsomhetsvurdering" onShow={onShow} /> og offentliggjør redegjørelse</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}><strong>Forbrukerrett:</strong> Alle kan kreve informasjon om leverandørkjedens arbeidsforhold</li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1589829545856-d10d557cf95f?auto=format&fit=crop&w=800&q=80" alt="Lovverk og regelverk" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Hvilken bærekraftsutfordring i din fremtidige arbeidsplass vil ha størst effekt å løse — og hvorfor er det nettopp der du bør starte?
        </h2>
        <p style={{ ...bodyText, fontSize: 20 }}>
          Prioriter Scope 3 og leverandørkjeden — det er der hoveddelen av miljøpåvirkningen faktisk skjer.
        </p>
      </div>
    </Glass>
  )
...
export default function BaerekraftVerdikjedePresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Bærekraft og verdikjede',
    }))
    setLivePin(pin)
