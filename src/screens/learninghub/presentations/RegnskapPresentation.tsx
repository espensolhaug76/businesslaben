...
const TERMS: Record<string, string> = {
  'Regnskap': 'Systematisk oversikt over alle inntekter og utgifter som viser hva som faktisk har skjedd i en periode.',
  'Budsjett': 'En plan for fremtidige inntekter og utgifter. Hva bedriften tror vil skje.',
  'Faste kostnader': 'Kostnader som ikke endrer seg med produksjonsvolumet, f.eks. husleie og lån.',
  'Variable kostnader': 'Kostnader som øker eller minker med produksjonsvolumet, f.eks. råvarer.',
  'Selvkostkalkylen': 'Beregning av hva en vare faktisk koster bedriften å produsere, inkludert alle kostnader.',
  'Dekningsbidrag': 'Salgspris minus variable kostnader. Pengene som er igjen til å dekke faste kostnader.',
  'Dekningsgrad': 'Dekningsbidrag i prosent av salgspris. Viser hvor mye av hver salgskrone som går til faste kostnader.',
  'Likviditet': 'Bedriftens evne til å betale sine løpende forpliktelser. Å ha penger tilgjengelig når man trenger dem.',
  'Lønnsomhet': 'Bedriftens evne til å tjene mer enn den bruker over tid.',
  'Resultatbudsjett': 'En oversikt som sammenstiller forventede inntekter og utgifter for å se om bedriften vil gå med overskudd.',
}
...
const QUIZ_SLIDES: QuizQuestion[] = [
  ...
  },
  {
    ...
    options: [
      'De øker proporsjonalt med produksjonsvolumet',
      'De endrer seg ikke med produksjonsvolumet',
      'De gjelder kun råvarekostnader',
      'De er alltid høyere enn variable kostnader',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva betyr likviditet for en bedrift?',
    options: [
      'Bedriftens lønnsomhet over tid',
      'Totale eiendeler i bedriften',
      'Bedriftens evne til å betale løpende forpliktelser',
      'Antall produkter solgt per måned',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Totalt antall kunder som er dekket av forsikringen',
      'Andelen av markedet bedriften dekker med sine produkter',
      'Dekningsbidraget i prosent av salgsprisen',
      'Prosentandelen av kostnader som dekkes av subsidier',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  {
    question: 'Hva er forskjellen mellom regnskap og budsjett?',
    options: [
      'Regnskap er for store bedrifter, budsjett er for små',
      'Regnskap viser hva som faktisk har skjedd, budsjett er en plan for fremtiden',
      'De er det samme dokumentet med ulikt navn',
      'Regnskap er fremoverrettet, budsjett er bakoverrettet',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Regnskap og<br />
        <span style={{ color: '#38bdf8', fontStyle: 'italic' }}>lønnsomhet</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>
        Forstå tallene som forteller om bedriften din overlever – eller ikke.
      </p>
...
function Slide1({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 5 – Regnskap</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Regnskap" onShow={onShow} /> vs. <Term word="Budsjett" onShow={onShow} />
          </h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}>
              <strong style={{ color: '#f1f5f9' }}>Regnskap:</strong> Viser hva som har skjedd (historien).
            </li>
            <li style={bodyText}>
              <strong style={{ color: '#f1f5f9' }}>Budsjett:</strong> En plan for hva du tror skal skje (fremtiden).
            </li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=800&q=80" alt="Regnskap" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1450101499163-c8848c66ca85?auto=format&fit=crop&w=800&q=80" alt="Kostnader" />
        <div>
          ...
          <p style={{ ...bodyText, marginBottom: 12 }}>
            Husleie er en fast kostnad (den endrer seg ikke om du selger lite eller mye).
          </p>
          <p style={bodyText}>
            Kjøttdeigen til en taco-restaurant er en variabel kostnad (du må kjøpe mer hvis du selger mer taco).
          </p>
        </div>
      </div>
    </Glass>
...
function Slide3({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Kostnadsberegning</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Selvkostkalkylen" onShow={onShow} />
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>En oppskrift på hva varen faktisk koster bedriften:</p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>+ Direkte materialkostnader</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>+ Direkte lønnskostnader</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>+ Indirekte kostnader</li>
              <li style={{ ...bodyText, color: '#38bdf8', fontWeight: 700, borderTop: '1px solid rgba(56,189,248,0.3)', paddingTop: 8, marginTop: 4 }}>= Selvkost</li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=800&q=80" alt="Kalkyle" />
      </div>
    </Glass>
  )
}
      ...
      <div style={twoCol}>
        ...
        <div>
          ...
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Det du sitter igjen med etter at du har betalt for selve varen. Disse pengene skal brukes til å betale husleia.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
    ...
    <Glass>
      <div style={twoCol}>
        <div>
          ...
          <p style={bodyText}>
            Viser dekningsbidraget i prosent. Hvis du har en DG på 40 %, betyr det at av hver hundrelapp du selger for, sitter du igjen med 40 kr til faste utgifter.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=800&q=80" alt="Prosent" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=800&q=80" alt="Penger" />
        <div>
          <SlideLabel>Betalingsevne</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Likviditet" onShow={onShow} /> – Har du penger nå?
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Du kan ha masse verdier (som biler og varelager), men hvis du ikke har penger på konto til å betale lønn i morgen, har du dårlig likviditet.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1rem 1.5rem' }}>
            <p style={{ ...bodyText, color: '#38bdf8', fontWeight: 700, textAlign: 'center' }}>"Cash is king"</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide7({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Planlegging</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Resultatbudsjett" onShow={onShow} />
          </h2>
          <p style={bodyText}>
            En tabell der du skriver opp hvor mye du håper å selge for, og hvor mye utgifter du forventer. Målet er å se om du går med overskudd til slutt.
          </p>
        </div>
...
function Slide8({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=800&q=80" alt="Lønnsomhet" />
        <div>
          <SlideLabel>Overlevelse</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Lønnsomhet" onShow={onShow} />
          </h2>
          <p style={bodyText}>
            En bedrift er lønnsom hvis de tjener mer penger enn de bruker over lang tid. Ellers går de konkurs.
          </p>
        </div>
      </div>
    </Glass>
      ...
      <div style={twoCol}>
        <div>
          ...
          <div style={{ ...bodyText, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem', marginBottom: 16 }}>
            Hvis en kaffe koster 45 kr, og bønner/melk koster 5 kr, er <Term word="Dekningsbidrag" onShow={onShow} /> 40 kr. Men kaffebaren må selge hundrevis av kopper hver dag for å betale de <Term word="Faste kostnader" onShow={onShow} /> som husleie på 40 000 kr!
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=800&q=80" alt="Kaffebar" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Hvorfor er det livsfarlig å bare se på dekningsbidraget og glemme husleia og strømregningen?
        </h2>
        <p style={{ ...bodyText, fontSize: 20 }}>
          Tenk på kaffebaren: høyt dekningsbidrag per kopp, men faste kostnader tar alle pengene hvis du selger for lite.
        </p>
      </div>
    </Glass>
  )
...
export default function RegnskapPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Regnskap og lønnsomhet',
    }))
    setLivePin(pin)
