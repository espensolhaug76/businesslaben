...
const TERMS: Record<string, string> = {
  'beredskap': 'Å være forberedt og klar til å håndtere en krise eller uønsket hendelse på en profesjonell og effektiv måte.',
  'beredskapsplan': 'En skriftlig plan som beskriver steg-for-steg hvem som gjør hva ved ulike krisesituasjoner som brann, ran, strømbrudd eller ulykker.',
  'evakuering': 'Prosessen der alle personer i bygningen forlater lokalene raskt og trygt og samles på et forhåndsbestemt møtepunkt.',
  'debrifing': 'En strukturert samtale etter en hendelse der alle involverte deler sine opplevelser, bearbeider inntrykk og trekker lærdommer.',
  'varsling': 'Å gi beskjed til riktig person eller instans (nødetater, ledelse, vekter) så raskt som mulig når noe kritisk skjer.',
}

const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva er en beredskapsplan?',
    options: [
      'En plan for bedriftens ordinære daglige drift og rutiner',
      'En plan for hvordan organisasjonen skal håndtere nødsituasjoner og kriser',
      'En markedsplan for å nå nye kundesegmenter',
      'En finansiell plan for neste regnskapsår',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Kun kontaktinformasjon til nødetatene',
      'Klare roller, kontaktlister, prosedyrer og kommunikasjonsplan',
      'Utelukkende en detaljert evakueringsplan',
      'Kun medisinsk beredskap og førstehjelpsrutiner',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hvorfor er øvelse viktig i beredskapsarbeid?',
    options: [
      'Øvelse er en unødvendig tidsbruk for travle virksomheter',
      'Øvelse tester og forbedrer planen slik at alle vet hva de skal gjøre',
      'Øvelse er kun nødvendig for å oppnå formell sertifisering',
      'Øvelse er frivillig og kun aktuelt for risikoutsatte bransjer',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er debrifing etter en krisesituasjon?',
    options: [
      'En skriftlig rapport levert til myndighetene',
      'En strukturert samtale der involverte deler opplevelser og lærer av hendelsen',
      'En teknisk gjennomgang av tekniske systemer etter svikt',
      'En risikovurdering som gjennomføres før neste hendelse',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hvem har ansvaret for å utarbeide en beredskapsplan i en bedrift?',
    options: [
      'De ansatte i fellesskap gjennom demokratisk avstemning',
      'Ledelsen, gjerne i samarbeid med verneombudet og de ansatte',
      'Kommunens beredskapssjef og brannvesenet',
      'Forsikringsselskapet som har bedriftens forsikring',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Beredskaps<span style={{ color: '#38bdf8', fontStyle: 'italic' }}>planer</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 7 – Vær klar når krisen inntreffer. Hvem gjør hva, og hvem ringer du?</p>
      {_isLive ? (
...
function Slide1({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1582719508461-905c673771fd?auto=format&fit=crop&w=800&q=80" alt="Beredskap og krisehåndtering" />
        <div>
          <SlideLabel>Modul 7 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Hva er <Term word="beredskap" onShow={show} />?</h2>
          <p style={bodyText}>Det betyr å være klar. Når alarmen går eller en krise oppstår, skal du ikke bruke tid på å lure på hva du skal gjøre – du skal vite nøyaktig hvordan du skal reagere profesjonelt.</p>
        </div>
      </div>
    </Glass>
  )
...
function Slide2({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 7 – Slide 2</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="beredskapsplan" onShow={show} /> – Oppskriften</h2>
          <p style={bodyText}>Dette er en skriftlig "kokebok" for kriser. Den beskriver steg-for-steg hvem som gjør hva ved brann, ulykker, ran, strømbrudd eller hvis datasystemene krasjer.</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=800&q=80" alt="Planlegging og dokumentasjon" />
      </div>
    </Glass>
  )
}
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1423666639041-f56000c27a9a?auto=format&fit=crop&w=800&q=80" alt="Nødtelefon og varsling" />
        <div>
          <SlideLabel>Modul 7 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="varsling" onShow={show} /> – Hvem ringer du?</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[['🔥 110', 'Brann'], ['🚔 112', 'Politi'], ['🚑 113', 'Ambulanse']].map(([num, label]) => (
              <div key={num} style={{ display: 'flex', alignItems: 'center', gap: 16, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 12, padding: '0.75rem 1.25rem' }}>
                <span style={{ color: '#38bdf8', fontWeight: 800, fontSize: 20, minWidth: 60 }}>{num}</span>
                <span style={{ color: '#cbd5e1', fontSize: 17 }}>{label}</span>
              </div>
            ))}
            <p style={{ ...bodyText, fontSize: 16, marginTop: 8 }}>Planen inneholder også interne numre til sjef, vekterselskap og fagfolk som rørlegger eller elektriker.</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 7 – Slide 4</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Handling og Ansvar</h2>
          <p style={bodyText}>Hvem har ansvaret for å telle gjestene på møteplassen ved <Term word="evakuering" onShow={show} />? Hvem har lov til å snakke med pressen? Tydelige roller i beredskapsplanen hindrer panikk og redder liv.</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1521791136064-7986c2920216?auto=format&fit=crop&w=800&q=80" alt="Teamarbeid og ansvar" />
      </div>
    </Glass>
  )
}
...
function Slide5() {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1560264280-88b68371db39?auto=format&fit=crop&w=800&q=80" alt="Trening og øvelse" />
        <div>
          <SlideLabel>Modul 7 – Slide 5</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Psykisk beredskap</h2>
          ...
          <div style={{ marginTop: 24, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.25rem' }}>
            <p style={{ color: '#38bdf8', fontWeight: 700, fontSize: 16 }}>En beredskapsplan er verdiløs hvis de ansatte aldri har øvd på den i praksis.</p>
          </div>
        </div>
      </div>
    </Glass>
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 7 – Slide 6 – Diskusjon</SlideLabel>
        ...
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '2px dashed rgba(56,189,248,0.3)', borderRadius: 24, padding: '3rem' }}>
          <p style={{ fontSize: 26, color: '#e2e8f0', lineHeight: 1.6, fontWeight: 300, fontStyle: 'italic' }}>
            "Hvorfor er det så viktig at de ansatte snakker sammen (<Term word="debrifing" onShow={show} />) rett etter at en dramatisk hendelse er over, selv om alt gikk bra til slutt?"
          </p>
        </div>
      </div>
    </Glass>
...
export default function BeredskapsplanerPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Beredskapsplaner',
    }))
    setLivePin(pin)
