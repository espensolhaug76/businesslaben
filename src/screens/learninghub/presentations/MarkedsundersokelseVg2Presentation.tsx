...
const TERMS: Record<string, string> = {
  'Primærdata': 'Data samlet inn direkte fra kilden for et spesifikt formål — fersk og skreddersydd, men kostbar og tidkrevende.',
  'Sekundærdata': 'Allerede eksisterende informasjon fra SSB, Kantar-rapporter eller bransjestatistikk — raskere og billigere, men kan være utdatert.',
  'Kvantitativ metode': 'Innsamling av målbare data fra mange respondenter for å trekke statistiske slutninger — gir bredde.',
  'Kvalitativ metode': 'Dybdeforståelse gjennom intervju, fokusgruppe og observasjon — gir innsikt i motivasjon og årsaker.',
  'Likert-skala': 'Skala fra 1 til 5 (svært uenig til svært enig) brukt til å måle holdninger og grad av enighet i spørreundersøkelser.',
  'NPS': 'Net Promoter Score (0–10) — mål på kundelojalitet og vilje til å anbefale bedriften til andre.',
  'Representativitet': 'Grad av samsvar mellom utvalgets sammensetning og den populasjonen man ønsker å si noe om.',
  'Bekreftelsesbias': 'Tendensen til å legge mer vekt på funn som støtter det man allerede tror, og ignorere funn som utfordrer det.',
  'Feilmargin': 'Det statistiske usikkerhetsintervallet i en undersøkelse — typisk ±3% for et representativt utvalg på 1000.',
  'Mystery shopping': 'Observasjonsmetode der testpersoner opptrer som vanlige kunder for å observere og rapportere faktisk servicekvalitet.',
}
...
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    ...
    options: [
      'Data fra SSB og andre offentlige registre',
      'Data samlet inn direkte fra kilden for ditt spesifikke formål',
      'Historiske data fra tidligere undersøkelser',
      'Data kjøpt fra analysebyrå',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Å rangere konkurrenter',
      'Å beregne NPS-score',
      'Å telle antall respondenter',
      'Å måle holdninger og grad av enighet/uenighet (1–5)',
    ],
    correct: 3,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Å bekrefte at dataene er korrekte',
      'Tendensen til å legge mer vekt på funn som støtter det man allerede tror',
      'Å bekrefte resultatene med en ny undersøkelse',
      'At flertallet av respondentene er enige',
    ],
    correct: 1,
    timeSeconds: 45,
  },
...
function Slide1({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Markedsundersøkelse — Fase 1</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Primærdata" onShow={onShow} /> vs. <Term word="Sekundærdata" onShow={onShow} />
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Svar finnes kanskje allerede — spørsmålet er om de er gode nok for deg.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Primærdata:</strong> Samles inn selv — fersk, relevant, men kostbar.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Sekundærdata:</strong> SSB, Kantar, bransjestatistikk — rask tilgang.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Kombinasjon:</strong> Start med sekundær, suppler med primær der det trengs.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=800&q=80" alt="Data" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="Kvantitativ" />
        <div>
          <SlideLabel>Kvantitative metoder</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Kvantitativ metode" onShow={onShow} /> — Tall gir bredde
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Bare riktig stilte spørsmål gir riktige tall. Spørreundersøkelsen er den vanligste formen.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Utvalgsstørrelse:</strong> Etter ca. 1000 flater forbedringen ut.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Spørsmålsdesign:</strong> Unngå ledende, doble eller tvetydige spørsmål.</li>
            <li style={bodyText}><Term word="Likert-skala" onShow={onShow} />: 1–5 for holdninger. <Term word="NPS" onShow={onShow} />: 0–10 for lojalitet.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide3({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Kvalitative metoder</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Kvalitativ metode" onShow={onShow} /> — Dybde
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Kvantitativt sier hva og hvor mange — kvalitativt sier <em>hvorfor</em>.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Dybdeintervju:</strong> Rike, personlige innsikter.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Fokusgruppe:</strong> Avdekker sosiale normer — pass på dominanseffekten.</li>
            <li style={bodyText}><Term word="Mystery shopping" onShow={onShow} />: Observasjon av faktisk atferd.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&w=800&q=80" alt="Intervju" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=800&q=80" alt="Representativitet" />
        <div>
          <SlideLabel>Utvalg og representativitet</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Representativitet" onShow={onShow} />
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Et skjevt utvalg gir sikre svar på feil spørsmål.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Tilfeldig utvalg:</strong> Alle har lik sjanse — gir best representativitet.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Convenience sampling:</strong> Praktisk men gir sjelden generaliserbare resultater.</li>
            <li style={bodyText}><Term word="Feilmargin" onShow={onShow} />: Representativt utvalg på 1000 gir typisk ±3%.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Tolkning av data</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Bekreftelsesbias" onShow={onShow} /> — Den største fellen
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Data er råvaren. Innsikt er produktet. Beslutning er målet.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#94a3b8' }}>"67% er misfornøyde" er <em>data</em>.</li>
              <li style={{ ...bodyText, color: '#f1f5f9', fontWeight: 600 }}>"Kunder forlater oss fordi de føler seg ignorert" er <em>innsikt</em>.</li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="Analyse" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=800&q=80" alt="Case NRK" />
        <div>
          <SlideLabel>Case — NRK</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Kombinasjon av metoder
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            NRK starter med Kantar-paneldata (<Term word="Sekundærdata" onShow={onShow} />) og bestiller deretter primærforskning for å fylle hullene.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1rem 1.5rem' }}>
            <p style={{ ...bodyText, color: '#38bdf8', fontWeight: 700, textAlign: 'center' }}>Sekundær gir overblikk, primær gir dybde</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide7({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Case — Vinmonopolet</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Kvalitativ innsikt som endrer alt
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Vinmonopolet brukte dybdeintervjuer og oppdaget at kunder ikke mislikte ventetiden — de mislikte følelsen av å bli <em>dømt</em>.
          </p>
          <p style={bodyText}>
            En spørreundersøkelse hadde aldri avdekket dette. <Term word="Kvalitativ metode" onShow={onShow} /> avdekker innsikter som talldata ikke kan gi.
          </p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&w=800&q=80" alt="Vinmonopolet" />
      </div>
    </Glass>
  )
}
...
function Slide8({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="Coop" />
        <div>
          <SlideLabel>Case — Coop og Rema 1000</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Datadrevet beslutningstaking
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Coop bruker lojalitetskortdata til å planlegge sortiment — atferdsdata som viser hva kunder faktisk kjøper, ikke hva de sier de kjøper.
          </p>
          <p style={bodyText}>
            Rema 1000 bruker paneler på 200 000+ for å teste produkter digitalt. Begge er eksempler på <Term word="Primærdata" onShow={onShow} /> i praksis.
          </p>
        </div>
      </div>
    </Glass>
...
function Slide9({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Presentasjon av funn</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Fra data til handling
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            En rapport uten handlingspunkter er arkivmateriale. Alltid avslutte med konkrete anbefalinger.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Innsikt først:</strong> Viktigste funn i første setning, deretter støttedata.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Hva nå?</strong> Tre konkrete anbefalinger med prioriteringsrekkefølge.</li>
            <li style={bodyText}><Term word="Bekreftelsesbias" onShow={onShow} />: Let aktivt etter funn som <em>motstrider</em> hypotesen din.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=800&q=80" alt="Rapport" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Hvorfor er det farlig å hoppe rett til løsningen uten å forstå problemet godt nok?
        </h2>
        <p style={{ ...bodyText, fontSize: 20 }}>
          En time med fem kunder kan spare måneder med feil retning. Markedsundersøkelse er ikke kostnad — det er risikoreduksjon.
        </p>
      </div>
    </Glass>
  )
...
export default function MarkedsundersokelseVg2Presentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Markedsundersøkelse',
    }))
    setLivePin(pin)
