...
const TERMS: Record<string, string> = {
  'Konflikttrappa': 'Modell som beskriver hvordan en konflikt eskalerer fra saklig uenighet til total krig i fire trinn.',
  'Deeskalering': 'Teknikker for å dempe en konflikt og bringe den tilbake til et rolig nivå.',
  'Beredskapsplan': 'Bedriftens plan for hva som skal gjøres i krise- og nødssituasjoner.',
  'Psykisk beredskap': 'Mental forberedelse på vanskelige situasjoner slik at man handler riktig under press.',
  'Debrifing': 'Gjennomgang og samtale etter en vanskelig hendelse for å bearbeide opplevelsen og lære av den.',
  'Nødutgang': 'Merket rømningsvei i et bygg som skal brukes ved evakuering.',
}
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva er det viktigste første steget i konflikthåndtering?',
    options: [
      'Å ta parti for den parten som virker mest fornuftig',
      'Å anerkjenne konflikten og skape et trygt rom for åpen dialog',
      'Å ignorere konflikten og håpe den løser seg av seg selv',
      'Å umiddelbart rapportere konflikten til politiet',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva kjennetegner en nødssituasjon?',
    options: [
      'En situasjon med ekstraordinært høy arbeidsbelastning',
      'En akutt situasjon som truer personers liv, helse eller sikkerhet',
      'En finansiell krise i bedriften eller organisasjonen',
      'En alvorlig konfliktsituasjon mellom kolleger',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er de-eskalering?',
    options: [
      'Å trappe opp og intensivere en konflikt for rask løsning',
      'Å bevisst redusere spenningen og intensiteten i en konflikt',
      'Å skalere opp virksomheten etter en krise',
      'Å avslutte en samtale og forlate situasjonen',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er konflikttrappa?',
    options: [
      'Et verktøy for å komme ut av en brannfarlig situasjon',
      'En modell som beskriver hvordan en konflikt eskalerer trinn for trinn',
      'En metode for å fordele arbeidsoppgaver mellom kolleger',
      'En kommunikajonsteknikk for å løse misforståelser',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er psykisk beredskap?',
    options: [
      'En plan for å håndtere psykiske helseproblemer blant ansatte',
      'Mental forberedelse på vanskelige situasjoner slik at man handler riktig under press',
      'Psykologisk behandling etter traumatiske arbeidsulykker',
      'En treningsmetode for å øke mentalt fokus i hverdagen',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Konflikt og<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>nødssituasjoner</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 23 – Håndtering av konflikter og nødssituasjoner som fagarbeider.</p>
...
function Slide1() {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 23 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Profesjonalitet under press</h2>
          <p style={bodyText}>Å beholde roen når kunden skriker eller når brannalarmen går, er kanskje din aller viktigste jobb som fagarbeider.</p>
          <div style={{ marginTop: 20, padding: '1.25rem 1.5rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 14 }}>
            <p style={{ color: '#38bdf8', fontWeight: 600, fontSize: 15, margin: 0 }}>Ro og profesjonalitet redder situasjoner</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1573497620053-ea5300f94f21?auto=format&fit=crop&w=800&q=80" alt="Profesjonalitet under press" />
      </div>
    </Glass>
  )
}
...
function Slide2({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Konflikttrappa</SlideLabel>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { step: '4', title: 'Krig', desc: 'Situasjonen er helt låst og vi ser på hverandre som fiender.', color: '#ef4444' },
              { step: '3', title: 'Sinte ord', desc: 'Vi blander sak og person og begynner å kalle hverandre ting.', color: '#f97316' },
              { step: '2', title: 'Krangling', desc: 'Vi slutter å lytte og står på vårt.', color: '#eab308' },
              { step: '1', title: 'Saklig uenighet', desc: 'Vi er uenige om en ting, men snakker sammen.', color: '#22c55e' },
            ].map(item => (
              <div key={item.step} style={{ padding: '0.75rem 1rem', background: 'rgba(255,255,255,0.05)', borderRadius: 12, borderLeft: `4px solid ${item.color}`, display: 'flex', gap: 12, alignItems: 'center' }}>
                <span style={{ fontWeight: 800, color: item.color, fontSize: 20, minWidth: 24 }}>{item.step}</span>
                <div>
          ...
          <p style={{ color: '#64748b', fontSize: 13, marginTop: 12, fontStyle: 'italic' }}>Klikk på <Term word="Konflikttrappa" onShow={show} /> for mer info</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1573497620053-ea5300f94f21?auto=format&fit=crop&w=800&q=80" alt="Konflikt" />
      </div>
    </Glass>
  )
}
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1563906267088-b029e7101114?auto=format&fit=crop&w=800&q=80" alt="Deeskalering" />
        <div>
          <SlideLabel>Modul 23 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Deeskalering" onShow={show} /> – Slukk brannen</h2>
          <p style={bodyText}>Bruk en rolig stemme, vis åpne hender og si: "Jeg forstår at du er frustrert, la oss finne en løsning".</p>
          <div style={{ marginTop: 20, padding: '1.25rem 1.5rem', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 14 }}>
            <p style={{ color: '#f87171', fontWeight: 600, fontSize: 15, margin: 0 }}>❌ Aldri si "Ro deg ned!"</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 23 – Slide 4</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Din rolle i nødssituasjoner</h2>
          <p style={bodyText}>Du må vite hvor <Term word="Nødutgang" onShow={show} />ene er og hvem som har ansvaret for hva i <Term word="Beredskapsplan" onShow={show} />en. Din jobb er å lede gjestene i sikkerhet.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Kjenn nødutgangene', 'Følg beredskapsplanen', 'Led gjestene rolig og trygt'].map(item => (
        ...
        <SlideImg src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=800&q=80" alt="Nødssituasjoner" />
      </div>
    </Glass>
  )
}
...
function Slide5() {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Aggressive personer" />
        <div>
          <SlideLabel>Modul 23 – Slide 5</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Aggressive personer</h2>
          <p style={bodyText}>Din sikkerhet kommer først! Ved trusler eller vold skal du trekke deg unna og tilkalle hjelp/vektere med en gang.</p>
          <div style={{ marginTop: 20, padding: '1.5rem', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 16 }}>
            <p style={{ color: '#fca5a5', fontWeight: 700, fontSize: 16, margin: 0 }}>Sikkerhet først – aldri leik helt!</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide6({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 23 – Slide 6</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Psykisk beredskap" onShow={show} /></h2>
          <p style={bodyText}>Vi trener på vanskelige situasjoner slik at hjernen vet hva den skal gjøre når det faktisk smeller.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Rollespill og øvelser', 'Kjenn prosedyrene utenat', 'Bevar roen gjennom forberedelse'].map(item => (
              <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#cbd5e1', fontSize: 16 }}>
                <span style={{ color: '#38bdf8', fontWeight: 800 }}>→</span> {item}
              </li>
            ))}
        ...
        <SlideImg src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=800&q=80" alt="Psykisk beredskap" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 23 – Slide 7 – Case</SlideLabel>
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Case: Butikktyveri</h2>
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 24, padding: '2.5rem' }}>
          <p style={{ ...bodyText, fontSize: 18, margin: 0 }}>Hvis du ser noen stjele, skal du aldri leke helt. Følg bedriftens regler – ofte er det bedre å observere og ringe politiet.</p>
        </div>
        <p style={{ color: '#64748b', fontSize: 15, marginTop: 20, fontStyle: 'italic' }}>Din sikkerhet er viktigere enn varene i butikken.</p>
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Hvorfor er det viktig at de ansatte snakker sammen etter at noe skummelt har skjedd?</h2>
        <p style={{ ...bodyText, fontSize: 20 }}>Hva er hensikten med <Term word="Debrifing" onShow={show} />, og hvordan kan det hjelpe både den enkelte og arbeidsplassen å bli bedre?</p>
      </div>
    </Glass>
  )
}

export default function KonfliktNodPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Konflikt og nødssituasjoner',
    }))
    setLivePin(pin)
