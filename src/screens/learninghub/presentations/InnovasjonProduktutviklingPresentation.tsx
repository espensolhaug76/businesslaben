...
const TERMS: Record<string, string> = {
  'Innovasjon': 'Frembringelsen av noe nytt som tas i bruk og skaper kommersiell eller samfunnsmessig verdi. Idé + Gjennomføring + Verdi.',
  'Inkrementell innovasjon': 'Kontinuerlige, trinnvise forbedringer av eksisterende produkter, tjenester eller prosesser — lavest risiko.',
  'Radikal innovasjon': 'Fundamentalt nye løsninger som endrer et marked eller skaper helt nye industrier — høy risiko og høy potensiell avkastning.',
  'Disrupsjon': 'Nye aktører med radikalt nye forretningsmodeller som utfordrer og erstatter etablerte spillere i en bransje.',
  'Produktutviklingsprosessen': 'Strukturert prosess fra idé til ferdig produkt: idégenerering, konseptutvikling, testing/validering og lansering.',
  'Prototype': 'En tidlig, ufullstendig versjon av et produkt brukt til å teste idéer og samle tilbakemelding fra potensielle kunder.',
  'LCA': 'Livssyklusanalyse — kartlegger klimaavtrykket til et produkt fra "vugge til grav", fra råvare til avfall.',
  'Triple bottom line': 'Rammeverk som krever at bedrifter vurderer tre dimensjoner: folk (People), planet (Planet) og profitt (Profit).',
  'Patent': 'Tidsbegrenset monopol på en teknisk oppfinnelse — gir eneretten til å utnytte oppfinnelsen i inntil 20 år.',
  'Innovasjon Norge': 'Statlig virkemiddelapparat som gir tilskudd og lån til norske bedrifter og prosjekter med innovasjonspotensial.',
}
...
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    ...
    options: [
      'Inkrementell forbedrer det eksisterende, radikal erstatter det med noe fundamentalt nytt',
      'Inkrementell er dyrere, radikal er billigere',
      'Inkrementell er for store bedrifter, radikal er for startups',
      'De er det samme, bare to ord for innovasjon',
    ],
    correct: 0,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Å spare penger på markedsføring',
      'Fordi loven krever testing av nye produkter',
      'Å avdekke problemer mens det er enkelt og billig å endre produktet',
      'Å generere PR og oppmerksomhet',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  {
    question: 'Airbnb er et eksempel på radikal innovasjon. Hva er det som gjør det radikalt?',
    options: [
      'Det er en ny app — teknologien er radikalt ny',
      'Det endret fundamentalt HVEM som tilbyr overnatting — fra hoteller til privatpersoner',
      'Det er billigere enn hotell',
      'Det bruker algoritmer til å sette priser',
    ],
    correct: 1,
    timeSeconds: 45,
  },
...
function Slide1({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Innovasjon — Fase 1</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Hva er <Term word="Innovasjon" onShow={onShow} />?
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            En idé er ikke innovasjon — det er starten på et arbeid. Innovasjon krever at idéen faktisk tas i bruk og skaper verdi.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1rem 1.5rem' }}>
            <p style={{ ...bodyText, color: '#38bdf8', fontWeight: 700, textAlign: 'center' }}>Innovasjon = Idé × Gjennomføring × Verdi</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80" alt="Innovasjon" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&w=800&q=80" alt="Inkrementell" />
        <div>
          <SlideLabel>Inkrementell innovasjon</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Inkrementell innovasjon" onShow={onShow} />
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            De fleste innovasjoner er ikke revolusjoner — de er trinnvise forbedringer. Scandic innfører digital innsjekking. En kafé tilbyr ny smaksvariant.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Fordeler:</strong> Lavere risiko, kjent kundebase, bygger på eksisterende kompetanse.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Ulemper:</strong> Kan ikke forsvare seg mot radikale forstyrrere.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide3({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Radikal innovasjon</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Disrupsjon" onShow={onShow} /> — Banebrytende endring
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Disrupsjon er ikke en ulykke — det er resultatet av at noen torde å tenke annerledes.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Airbnb:</strong> Disrupterte hotellbransjen — P2P-utleie.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Uber:</strong> Disrupterte taxi med ny forretningsmodell.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Spotify:</strong> Disrupterte platebutikkene.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80" alt="Disrupsjon" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&w=800&q=80" alt="Produktutvikling" />
        <div>
          <SlideLabel>Produktutviklingsprosessen</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Fra idé til marked
          </h2>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>1. Idégenerering:</strong> Brainstorm, kundeintervjuer, benchmarking.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>2. Konseptutvikling:</strong> Velg lovende idéer, lag <Term word="Prototype" onShow={onShow} />.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>3. Testing og validering:</strong> Test på utvalgte kunder, iterer.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>4. Lansering:</strong> Rull ut med kampanje og oppfølging.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Bærekraft i produktutvikling</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Triple bottom line" onShow={onShow} />
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Fremtidens kunder vil straffe produkter som ikke tar hensyn til klima og etikk. Alle nye produkter må vurderes mot tre krav.
          </p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>Folk (People) — hvem produserer, under hvilke forhold?</li>
              <li style={{ ...bodyText, color: '#f1f5f9' }}>Planet — <Term word="LCA" onShow={onShow} /> fra vugge til grav.</li>
              <li style={{ ...bodyText, color: '#38bdf8', fontWeight: 700 }}>Profitt — kommersiell levedyktighet.</li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="Baerekraft" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow: _onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&w=800&q=80" alt="Scandic" />
        <div>
          <SlideLabel>Case — Scandic Hotels</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Digital innsjekking og minibarer
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Scandic innførte innsjekkingskiosker og fjernet minibarene — begge endringer reduserte energiforbruk og driftskostnader.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Kiosker:</strong> Frigjør personale til mer verdifull service.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Minibarer fjernet:</strong> Mer energikostnad enn omsetning.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Type:</strong> Inkrementell innovasjon — prosessen endres, kjernen består.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide7({ onShow: _onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Case — Trysil</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Fra alpinbakke til helårsdestinasjon
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Det smarteste Trysil gjorde var å ikke prøve å bli Åre — men å bli et bedre Trysil.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Problemet:</strong> Klimaendringer truer snøsesongen.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Løsningen:</strong> Norges beste stisykkelnett (200+ km).</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Resultatet:</strong> Sommerbelegg nær vinterbelegg.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&w=800&q=80" alt="Trysil" />
      </div>
    </Glass>
  )
}
...
function Slide8({ onShow: _onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80" alt="Myter" />
        <div>
          <SlideLabel>Vanlige misforståelser</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            Innovasjon krever ikke ny teknologi
          </h2>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Myte:</strong> Innovasjon = teknologi. Feil — nye serviceformer teller.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Myte:</strong> Bare store selskaper innoverer. Feil — lokal kafé med takeaway-abonnement innoverer.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Myte:</strong> Idéen er nok. Feil — uten gjennomføring er det bare kreativitet.</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide9({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Lovverk og støtte</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>
            <Term word="Patent" onShow={onShow} /> og <Term word="Innovasjon Norge" onShow={onShow} />
          </h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>
            Immaterielle rettigheter er ikke et juridisk detaljspørsmål — det er verdier på balansen.
          </p>
          <ul style={listStyle}>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Patent:</strong> Teknisk løsning — maks 20 år monopol.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Varemerke:</strong> Navn og logo — kan fornyes 10 år av gangen.</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>SkatteFUNN:</strong> Skattefradrag for FoU-kostnader.</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="Patent" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Hvilken innovasjon ville du foreslå for din skoles kantineordning?
        </h2>
        <p style={{ ...bodyText, fontSize: 20 }}>
          Gå gjennom hele produktutviklingsprosessen: idégenerering, konsept, prototype, testing og lansering.
        </p>
      </div>
    </Glass>
  )
...
export default function InnovasjonProduktutviklingPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Innovasjon og produktutvikling',
    }))
    setLivePin(pin)
