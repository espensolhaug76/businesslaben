...
const TERMS: Record<string, string> = {
  'Klagehåndtering': 'Systematisk prosess for å motta, behandle og løse kundeklager på en profesjonell måte.',
  'LYTT-metoden': 'Klagehåndteringsmetode: Lytt, beKlag, løs, følg opp Tilbake – fire trinn for god klagebehandling.',
  'Service Recovery Paradox': 'Fenomenet der en kunde som har fått en klage løst eksemplarisk, blir mer lojal enn om feilen aldri hadde skjedd.',
  'Empati': 'Evnen til å sette seg inn i en annens situasjon og forstå deres følelser.',
  'Forebygging': 'Tiltak som settes inn for å hindre at problemer og klager oppstår.',
  'Forbrukerkjøpsloven': 'Loven som gir forbrukere rettigheter ved kjøp av varer, inkludert rett til reparasjon, ombytte eller pengene tilbake.',
}
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hvorfor er god klagebehandling strategisk viktig for en bedrift?',
    options: [
      'Utelukkende fordi lovgivningen krever det',
      'Fordi en håndtert klage kan gjøre en misfornøyd kunde til en lojal ambassadør',
      'Fordi det er langt billigere enn all annen markedsføring',
      'Fordi det er obligatorisk opplæringsaktivitet for ansatte',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er det aller viktigste i starten av en klagesamtale?',
    options: [
      'Å umiddelbart forklare hva som gikk galt og hvem som har skylden',
      'Å lytte aktivt og vise genuin empati for kundens frustrasjon',
      'Å tilby kompensasjon eller rabatt før kunden har snakket ferdig',
      'Å sette kunden på vent mens du undersøker saken',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er kompensasjon i klagebehandling?',
    options: [
      'En straff eller gebyr pålagt den klagende kunden',
      'En form for godtgjørelse til kunden for ulempen de har opplevd',
      'En ekstra avgift for spesielle servicetjenester',
      'En bindende servicekontrakt for fremtidige kjøp',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er Service Recovery Paradox?',
    options: [
      'En situasjon der dårlig service aldri kan repareres',
      'Fenomenet der en eksemplarisk løst klage kan gjøre kunden mer lojal enn om feilen aldri skjedde',
      'Et paradoks der god service fører til høyere klagefrekvens',
      'En metode for å unngå klager gjennom forebyggende tiltak',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er de fire stegene i LYTT-metoden?',
    options: [
      'Lytt, Yte, Tilpass, Tilby',
      'Lær, Yrke, Tid, Tilbakemelding',
      'Lytt, beKlag, løs, følg opp Tilbake',
      'Løs, Yte, Tilby, Tilpass',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Klage-<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>håndtering</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 24 – Klagehåndtering og konfliktforebygging i praksis.</p>
      {_isLive ? (
...
function Slide1({ show: _show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 24 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Klagen er en gave</h2>
          <p style={bodyText}>En misfornøyd kunde som klager, gir deg en sjanse til å fikse feilen. De som er stille, kommer bare aldri tilbake og sprer dårlig rykte.</p>
          <div style={{ marginTop: 20, padding: '1.25rem 1.5rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 14 }}>
            <p style={{ color: '#38bdf8', fontWeight: 600, fontSize: 15, margin: 0 }}>En klage = En mulighet til å bli bedre</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Klagehåndtering" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 900, margin: '0 auto' }}>
        <SlideLabel>Metoden (LYTT)</SlideLabel>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginTop: 24 }}>
          {[
            { step: 'L', title: 'Lytt', desc: 'Lytt uten å avbryte. Kunden må få tømme seg.' },
            { step: 'Y', title: 'Beklag (Ja)', desc: 'Beklag for opplevelsen. Vis empati.' },
            { step: 'T', title: 'Løs', desc: 'Løs problemet sammen med kunden.' },
            { step: 'T', title: 'Følg opp', desc: 'Følg opp etterpå for å sjekke at alt er greit.' },
          ].map((item, i) => (
            <div key={i} style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 20, padding: '2rem', textAlign: 'center' }}>
              <div style={{ fontSize: 48, fontWeight: 800, color: '#38bdf8', marginBottom: 8 }}>{item.step}</div>
              <div style={{ fontWeight: 700, color: '#fff', fontSize: 18, marginBottom: 8 }}>{item.title}</div>
              <div style={{ color: '#94a3b8', fontSize: 15, lineHeight: 1.5 }}>{item.desc}</div>
            </div>
          ))}
        </div>
        <p style={{ color: '#64748b', fontSize: 13, marginTop: 16, fontStyle: 'italic' }}>Klikk på <Term word="LYTT-metoden" onShow={show} /> for mer info</p>
      </div>
    </Glass>
  )
}
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=800&q=80" alt="Service Recovery Paradox" />
        <div>
          <SlideLabel>Modul 24 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Service Recovery Paradox" onShow={show} /></h2>
          <p style={bodyText}>Hvis du løser en klage kjempebra, blir kunden ofte mer lojal enn om feilen aldri hadde skjedd i det hele tatt!</p>
        </div>
      </div>
    </Glass>
  )
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 24 – Slide 4</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Forebygging" onShow={show} /> – Unngå bråk</h2>
          <p style={bodyText}>Gi tydelig info om åpningstider og priser. Misforståelser oppstår ofte når kunden har feil forventninger.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Tydelig info om priser', 'Klare åpningstider', 'Realistiske forventninger'].map(item => (
        ...
        <SlideImg src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=800&q=80" alt="Forebygging" />
      </div>
    </Glass>
  )
}
...
function Slide5({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1521791136064-7986c2920216?auto=format&fit=crop&w=800&q=80" alt="Kundens rettigheter" />
        <div>
          <SlideLabel>Modul 24 – Slide 5</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Kundens rettigheter</h2>
          <p style={bodyText}><Term word="Forbrukerkjøpsloven" onShow={show} /> forteller nøyaktig hva kunden har krav på (f.eks. ny vare eller pengene tilbake).</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Rett til reparasjon', 'Rett til ombytte', 'Rett til pengene tilbake'].map(item => (
              <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#cbd5e1', fontSize: 16 }}>
                <span style={{ color: '#38bdf8', fontWeight: 800 }}>§</span> {item}
              </li>
            ))}
...
function Slide6({ show: _show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 24 – Slide 6</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Vanskelige kunder</h2>
          <p style={bodyText}>Noen klager bare for å være slemme. Du skal fortsatt være profesjonell og høflig, men du kan sette grenser og hente sjefen hvis det blir for drøyt.</p>
          <div style={{ marginTop: 20, padding: '1.25rem 1.5rem', background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.25)', borderRadius: 14 }}>
            <p style={{ color: '#fbbf24', fontWeight: 600, fontSize: 15, margin: 0 }}>Profesjonell + grenser + sjef ved behov</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1573497620053-ea5300f94f21?auto=format&fit=crop&w=800&q=80" alt="Vanskelige kunder" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 24 – Slide 7 – Case</SlideLabel>
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Case: Restaurant</h2>
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 24, padding: '2.5rem' }}>
          <p style={{ ...bodyText, fontSize: 18, margin: 0 }}>Hvis maten er kald, skal du ikke krangle. Beklag med en gang, hent ny varm mat, og gi dem kanskje en gratis dessert som plaster på såret.</p>
        </div>
        <p style={{ color: '#64748b', fontSize: 15, marginTop: 20, fontStyle: 'italic' }}>Resultat: En fornøyd gjest som forteller det videre.</p>
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Hvordan kan vi bruke klager til å faktisk bli en bedre bedrift?</h2>
        <p style={{ ...bodyText, fontSize: 20 }}>Tenk på systemer for å registrere og analysere klager, og hvordan tilbakemeldinger kan drive forbedring i hele organisasjonen.</p>
      </div>
    </Glass>
  )
}

export default function KlagehåndteringPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Klagehåndtering',
    }))
    setLivePin(pin)
