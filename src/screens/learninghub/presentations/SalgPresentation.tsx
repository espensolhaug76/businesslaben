...
const TERMS: Record<string, string> = {
  'Salgsprosessen': 'De seks fasene i et profesjonelt salg: planlegging, kontakt, behovsanalyse, argumentasjon, avslutning og oppfølging.',
  'Behovsanalyse': 'Den fasen der selgeren stiller spørsmål for å forstå kundens egentlige behov og ønsker.',
  'Åpne spørsmål': 'Spørsmål som begynner med hvem, hva, hvordan eller hvorfor og krever mer enn et ja/nei-svar.',
  'Egenskap': 'En teknisk beskrivelse av produktet (f.eks. "20 000 vannsøyle").',
  'Fordel': 'Hva egenskapen betyr for kunden i praksis (f.eks. "du blir ikke våt i regnet").',
  'Innvending': 'En motforestilling fra kunden mot å kjøpe. En god selger møter dette med argumenter.',
  'Avslutning': 'Fasen der selgeren ber kunden om å kjøpe. Krever mot og direkte kommunikasjon.',
  'Oppfølging': 'Kontakt med kunden etter salget for å sikre tilfredshet og bygge lojalitet.',
}
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva er behovsanalyse i salgsprosessen?',
    options: [
      'En analyse av konkurrentenes produkter og priser',
      'Fasen der selgeren kartlegger kundens behov gjennom aktiv lytting og spørsmål',
      'En finansiell analyse av kundens betalingsevne',
      'En markedsundersøkelse om potensielle kunder',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er forskjellen mellom egenskap og fordel i salgspresentasjonen?',
    options: [
      'Det er ingen meningsfull forskjell i praksis',
      'Egenskap beskriver produktet teknisk, fordel beskriver hva det betyr for kunden',
      'Fordel er den tekniske spesifikasjonen, egenskap er kundeopplevelsen',
      'Egenskap er alltid viktigere å kommunisere enn fordelen',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er en innvending i salgsprosessen?',
    options: [
      'En positiv tilbakemelding og kjøpssignal fra kunden',
      'Et motargument fra kunden som selgeren må håndtere konstruktivt',
      'En avslutningsstrategi for å lukke salget',
      'Et pristilbud fra konkurrerende selger',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er hensikten med oppfølging etter et salg?',
    options: [
      'Å sende ut markedsføring for fremtidige produkter',
      'Å sikre kundetilfredshet og bygge lojalitet for fremtidige kjøp',
      'Å kontrollere at betalingen er mottatt korrekt',
      'Å registrere salget i bedriftens regnskapssystem',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hvorfor er åpne spørsmål viktig i behovsanalysen?',
    options: [
      'Fordi de er raskere å besvare enn lukkede spørsmål',
      'Fordi de lar kunden fortelle fritt og gir mer informasjon om behovene',
      'Fordi de er enklere å formulere for selgeren',
      'Fordi de alltid fører direkte til et kjøp',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Salg er<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>problemløsning</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 15 – Profesjonelt salg fra første kontakt til lojal kunde.</p>
      {_isLive ? (
...
function Slide1() {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 15 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Salg er problemløsning</h2>
          <p style={bodyText}>En profesjonell selger "pusher" ikke ting på folk. Han eller hun hjelper kunden med å finne det produktet som løser deres behov.</p>
        </div>
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 900, margin: '0 auto' }}>
        <SlideLabel>Salgsprosessens 6 faser</SlideLabel>
        <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 8, lineHeight: 1.1 }}><Term word="Salgsprosessen" onShow={show} />s 6 faser</h2>
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', flexWrap: 'wrap', marginTop: 24 }}>
          {['1. Planlegging', '2. Kontakt', '3. Behovsanalyse', '4. Argumentasjon', '5. Avslutning', '6. Oppfølging'].map((fase, i) => (
            <div key={i} style={{ padding: '1rem 1.5rem', background: i === 2 ? 'rgba(56,189,248,0.15)' : 'rgba(255,255,255,0.05)', border: i === 2 ? '2px solid #38bdf8' : '1px solid rgba(255,255,255,0.1)', borderRadius: 16, fontSize: 15, fontWeight: 600, color: i === 2 ? '#38bdf8' : '#cbd5e1' }}>
              {fase}
            </div>
          ))}
        </div>
        <p style={{ ...bodyText, marginTop: 32, fontSize: 16 }}>Planlegging → Kontakt → Behovsanalyse → Argumentasjon → Avslutning → Oppfølging</p>
      </div>
    </Glass>
  )
}
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=800&q=80" alt="Lytte og behovsanalyse" />
        <div>
          <SlideLabel>Modul 15 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Behovsanalyse" onShow={show} /> – Kunsten å lytte</h2>
          <p style={bodyText}>Den viktigste fasen! Her må du stille <Term word="Åpne spørsmål" onShow={show} /> (Hvem, hva, hvorfor). En god selger snakker 30 % og lytter 70 % av tiden.</p>
        </div>
      </div>
    </Glass>
  )
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 900, margin: '0 auto' }}>
        <SlideLabel>Modul 15 – Slide 4</SlideLabel>
        <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 32, lineHeight: 1.1 }}><Term word="Egenskap" onShow={show} /> vs. <Term word="Fordel" onShow={show} /></h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
          <div style={{ padding: '2rem', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)', borderRadius: 20 }}>
            <div style={{ fontWeight: 800, color: '#f87171', fontSize: 18, marginBottom: 16 }}>Egenskap (Unngå!)</div>
            <p style={{ color: '#cbd5e1', fontSize: 16, lineHeight: 1.6, margin: 0, fontStyle: 'italic' }}>"Denne jakka har 20 000 vannsøyle"</p>
          </div>
          <div style={{ padding: '2rem', background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.25)', borderRadius: 20 }}>
            <div style={{ fontWeight: 800, color: '#4ade80', fontSize: 18, marginBottom: 16 }}>Fordel (Bruk dette!)</div>
            <p style={{ color: '#cbd5e1', fontSize: 16, lineHeight: 1.6, margin: 0, fontStyle: 'italic' }}>"Du kan leke ute i øsregn uten å bli våt"</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide5({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 15 – Slide 5</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Innvending" onShow={show} /> – "Det er for dyrt"</h2>
          <p style={bodyText}>Ikke gi opp! Forklar kunden hvorfor prisen er slik (f.eks. bedre holdbarhet eller miljøvennlig produksjon).</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Håndtere innvendinger" />
      </div>
    </Glass>
  )
}
...
function Slide6({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=800&q=80" alt="Salgsavslutning" />
        <div>
          <SlideLabel>Modul 15 – Slide 6</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Avslutning" onShow={show} /> – Tør du å spørre?</h2>
          <p style={bodyText}>Du må spørre kunden om de vil kjøpe:</p>
          <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
            {['"Skal jeg pakke denne inn til deg?"', '"Skal vi sette opp en bestilling?"'].map(q => (
              <div key={q} style={{ padding: '1rem 1.5rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 14, color: '#38bdf8', fontWeight: 600, fontStyle: 'italic' }}>{q}</div>
            ))}
          </div>
        </div>
      </div>
...
function Slide7({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 15 – Slide 7</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Oppfølging" onShow={show} /> – Kunden kommer tilbake</h2>
          <p style={bodyText}>Send en melding etter en uke og spør om de er fornøyde. Da føler kunden seg sett og vil handle hos deg igjen.</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?auto=format&fit=crop&w=800&q=80" alt="Kundeoppfølging" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 15 – Slide 8 – Case</SlideLabel>
        <h2 style={{ fontSize: 52, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Case: Optiker</h2>
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 24, padding: '2.5rem' }}>
          <p style={{ ...bodyText, fontSize: 18 }}>En flink selger i en brillebutikk bruker <Term word="Behovsanalyse" onShow={show} /> til å finne en innfatning som ikke bare ser kul ut, men som også sitter perfekt på kundens hode og passer deres livsstil.</p>
        </div>
        <p style={{ color: '#64748b', fontSize: 15, marginTop: 20, fontStyle: 'italic' }}>Resultat: En fornøyd kunde som kommer tilbake.</p>
      </div>
    </Glass>
  )
}

export default function SalgPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Salg er problemløsning',
    }))
    setLivePin(pin)
