...
const TERMS: Record<string, string> = {
  'Distribusjonskanal': 'Hele kjeden av aktører og aktiviteter som fører produktet fra produsenten til den endelige forbrukeren — inkludert grossister, detaljister og logistikk.',
  'Direkte distribusjon': 'Salg fra produsent direkte til sluttkunde uten mellomledd. Gir full kontroll over pris, presentasjon og kundedata. Eksempel: Tesla, IKEA.no, gårdsutsalg.',
  'Indirekte distribusjon': 'Salg via ett eller flere mellomledd (grossist, agent, forhandler). Gir bredere dekning til lavere kostnad per salgspunkt, men redusert kontroll.',
  'Mellomledd': 'Aktører mellom produsent og forbruker — grossister, distributører, agenter og detaljister. De transformeres av digitalisering, men forsvinner sjelden.',
  'Intensiv distribusjon': 'Maksimal tilgjengelighet i flest mulig salgspunkter. Passer for lavpris convenience-produkter som kjøpes impulsivt (Coca-Cola, Red Bull, tyggegummi).',
  'Selektiv distribusjon': 'Et begrenset antall utvalgte forhandlere som oppfyller kvalitetskrav. Balanse mellom dekning og kontroll. Brukes av Apple, Nike og BMW.',
  'Eksklusiv distribusjon': 'Svært få eller én forhandler per geografisk område. Skaper kunstig knapphet, opprettholder luksusprestige og gir full kontroll (Rolex, Ferrari).',
  'Omnikanal': 'Alle salgskanaler er integrerte og sømløse — kunden kan begynne kjøpet på nett, fortsette i app og avslutte i butikk. Motsetning til multikanal der kanalene er separate.',
}

const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Tesla selger utelukkende via egne showroom og nettside — ingen forhandlere. Hva kalles denne distribusjonsformen?',
    options: [
  ...
  {
    question: 'Rolex selger kun via nøye utvalgte juvelerere — svært få utsalgssteder per by. Hva er distribusjonsstrategien?',
    options: [
      'Intensiv distribusjon — for å nå flest mulig kunder',
      'Selektiv distribusjon — utvalgte forhandlere',
      'Eksklusiv distribusjon — kunstig knapphet og prestisje',
      'Direkte distribusjon — ingen mellomledd',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  {
    question: 'Hva er den viktigste forskjellen mellom omnikanal og multikanal?',
    options: [
      'Omnikanal bruker kun netthandel; multikanal bruker kun fysiske butikker',
      'Omnikanal har integrerte kanaler med sømløs kundeopplevelse; multikanal har separate kanaler',
      'Multikanal er nyere og mer avansert enn omnikanal',
      'Det er ingen reell forskjell — begge begrepene betyr det samme',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Distribusjon<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>— Plass</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Fra produsent til forbruker — kanaler, strategier og hvorfor beliggenhet fortsatt avgjør alt.</p>
    ...
    <Glass>
      <div style={twoCol}>
        <div>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>Distribusjon — det 3. konkurransemidlet</h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>En <Term word="Distribusjonskanal" onShow={onShow} /> er hele kjeden fra produsent til forbruker. Valget av kanal avgjør hvem som kjøper, til hvilken pris og med hvilken kundeopplevelse.</p>
          <p style={bodyText}>Digitalisering eliminerer sjelden <Term word="Mellomledd" onShow={onShow} /> — den transformerer dem. Reisebyråer ble til Booking.com. Platebransjen ble til Spotify.</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=800&q=80" alt="Distribusjon og logistikk" />
      </div>
    </Glass>
  )
}
...
function Slide2({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?auto=format&fit=crop&w=800&q=80" alt="Direkte vs indirekte distribusjon" />
        <div>
          <SlideLabel>Kanalstruktur</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}><Term word="Direkte distribusjon" onShow={onShow} /> vs. <Term word="Indirekte distribusjon" onShow={onShow} /></h2>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#22c55e' }}>Direkte:</strong> Produsent → Forbruker. Full kontroll over pris, presentasjon og kundedata. Høyere kostnad per kundekontakt. (Tesla, IKEA.no)</li>
            <li style={bodyText}><strong style={{ color: '#38bdf8' }}>Indirekte:</strong> Produsent → Mellomledd → Forbruker. Bredere dekning, lavere kostnad per salgspunkt — men redusert kontroll. (Orkla via Rema 1000)</li>
          </ul>
        </div>
      </div>
    </Glass>
...
function Slide3({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Mellomleddene</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>Grossist, detaljist og agent</h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>De tre klassiske <Term word="Mellomledd" onShow={onShow} />-typene tilfører hver sin verdi i kjeden:</p>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 10 }}><strong style={{ color: '#f1f5f9' }}>Grossist:</strong> Kjøper store volum fra produsent og selger til detaljister. (ASKO til Rema 1000)</li>
            <li style={{ ...bodyText, marginBottom: 10 }}><strong style={{ color: '#f1f5f9' }}>Detaljist:</strong> Selger direkte til forbrukere. (Power, H&M, din lokale kiosk)</li>
            <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>Agent:</strong> Formidler salg uten å eie varen. (Reisebyrå, eiendomsmegler)</li>
          </ul>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=800&q=80" alt="Grossist og detaljist" />
      </div>
    </Glass>
  )
}
...
function Slide4({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Intensiv distribusjon" />
        <div>
          <SlideLabel>Distribusjonsgrad — strategi 1</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}><Term word="Intensiv distribusjon" onShow={onShow} /></h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>Maksimal tilgjengelighet i flest mulig salgspunkter. Passer for lavpris convenience-produkter som kjøpes impulsivt.</p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1rem 1.5rem' }}>
            <p style={{ ...bodyText, color: '#38bdf8', fontWeight: 700, marginBottom: 8 }}>Eksempler:</p>
            <ul style={listStyle}>
              <li style={bodyText}>Coca-Cola — i alle butikker, bensinstasjoner og kantiner</li>
              <li style={bodyText}>Red Bull — der folk er sultne og trøtte</li>
              <li style={bodyText}>Tyggegummi — alltid i kassen</li>
            </ul>
          </div>
        </div>
      </div>
...
function Slide5({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Distribusjonsgrad — strategi 2</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}><Term word="Selektiv distribusjon" onShow={onShow} /></h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>Et begrenset antall utvalgte forhandlere som oppfyller spesifikke kvalitetskrav. Balanserer dekning mot kontroll over merkevarekommunikasjonen.</p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1rem 1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, marginBottom: 6 }}><strong style={{ color: '#f1f5f9' }}>Apple:</strong> Via Apple Store og godkjente forhandlere</li>
              <li style={{ ...bodyText, marginBottom: 6 }}><strong style={{ color: '#f1f5f9' }}>Nike:</strong> Utvalgte sportsbutikker med kompetent personell</li>
              <li style={bodyText}><strong style={{ color: '#f1f5f9' }}>BMW:</strong> Autoriserte forhandlere med servicekrav</li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1491553895911-0055eca6402d?auto=format&fit=crop&w=800&q=80" alt="Selektiv distribusjon" />
      </div>
    </Glass>
  )
}
...
function Slide6({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1594736797933-d0501ba2fe65?auto=format&fit=crop&w=800&q=80" alt="Eksklusiv distribusjon luksus" />
        <div>
          <SlideLabel>Distribusjonsgrad — strategi 3</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}><Term word="Eksklusiv distribusjon" onShow={onShow} /></h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>Svært få — ofte én — forhandler per geografisk område. Skaper kunstig knapphet som opprettholder luksusprestisje og høy pris.</p>
          <div style={{ background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.3)', borderRadius: 16, padding: '1rem 1.5rem' }}>
            <p style={{ ...bodyText, color: '#a855f7', fontWeight: 700, marginBottom: 4 }}>«Coca-Cola er overalt. Rolex er nesten ingensteds. Begge strategier er rette for sine merkevarer.»</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide7({ onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Digital distribusjon</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}><Term word="Omnikanal" onShow={onShow} /> vs. multikanal</h2>
          <p style={{ ...bodyText, marginBottom: 16 }}>Kunden tenker ikke i kanaler — de forventer en sømløs opplevelse overalt.</p>
          <ul style={listStyle}>
            <li style={{ ...bodyText, marginBottom: 12 }}><strong style={{ color: '#94a3b8' }}>Multikanal:</strong> Selger via nett OG butikk, men kanalene er separate siloer</li>
            <li style={bodyText}><strong style={{ color: '#38bdf8' }}><Term word="Omnikanal" onShow={onShow} />:</strong> Alle kanaler integrerte — begynner kjøpet på nett, henter i butikk, returnerer via app</li>
          </ul>
          <div style={{ marginTop: 16, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 12, padding: '0.75rem 1.25rem' }}>
            <p style={{ ...bodyText, color: '#38bdf8', fontWeight: 600, fontSize: 15 }}>IKEA og Zalando — norske ledere i omnikanal</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1563013544-824ae1b704d3?auto=format&fit=crop&w=800&q=80" alt="Digital omnikanal" />
      </div>
    </Glass>
  )
}
...
function Slide8({ onShow: _onShow }: { onShow: (title: string, text: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1611532736597-de2d4265fba3?auto=format&fit=crop&w=800&q=80" alt="Norsk eksempel IKEA" />
        <div>
          <SlideLabel>Norsk eksempel</SlideLabel>
          <h2 style={{ fontSize: 'clamp(1.5rem, 3vw, 2.5rem)', fontWeight: 800, marginBottom: 16, color: '#fff' }}>Tesla vs. Toyota — to distribusjonsfilosofier</h2>
          <div style={{ ...bodyText, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem', marginBottom: 16 }}>
            <p style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#f1f5f9' }}>Tesla:</strong> 100 % direkte — egne showroom og nettside. Vet nøyaktig hvem som kjøper hva. Full kontroll over pris og opplevelse.</p>
            <p style={bodyText}><strong style={{ color: '#f1f5f9' }}>Toyota:</strong> Indirekte via 8 000+ forhandlere globalt. Massiv dekning, men avhengig av forhandlerens servicekvalitet.</p>
          </div>
          <p style={bodyText}>Volvo startet direkte netthandel i 2021. Bransjen er i transformasjon.</p>
        </div>
      </div>
    </Glass>
  )
    ...
    <Glass>
      <div style={twoCol}>
        <div>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>Digitalisering dreper ikke mellomledd</h2>
          <p style={{ ...bodyText, marginBottom: 16 }}><Term word="Mellomledd" onShow={onShow} /> elimineres sjelden av digital teknologi — de <em>transformeres</em>. En ny type mellomledd oppstår og erstatter den gamle.</p>
          <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, padding: '1.5rem' }}>
            <ul style={listStyle}>
              <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#94a3b8' }}>Reisebyrå</strong> → <strong style={{ color: '#38bdf8' }}>Booking.com</strong></li>
              <li style={{ ...bodyText, marginBottom: 8 }}><strong style={{ color: '#94a3b8' }}>Platebransjen</strong> → <strong style={{ color: '#38bdf8' }}>Spotify</strong></li>
              <li style={bodyText}><strong style={{ color: '#94a3b8' }}>Taxiselskap</strong> → <strong style={{ color: '#38bdf8' }}>Uber</strong></li>
            </ul>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80" alt="Digital transformasjon" />
      </div>
    </Glass>
  )
}
    ...
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>
          Hvilken distribusjonsstrategi ville du valgt — og hvorfor?
        </h2>
        <p style={{ fontSize: 20, color: '#cbd5e1', lineHeight: 1.7 }}>
          Tenk på en bedrift du kjenner godt. Bruker de intensiv, selektiv eller eksklusiv distribusjon? Er det riktig valg for merkevaren deres — og hva ville skjedd om de byttet strategi?
        </p>
      </div>
    </Glass>
  )
...
export default function DistribusjonPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({ pin, currentSlide: 0, activeQuestion: null, questionRevealed: false, status: 'active', presentationName: 'Distribusjon — Plass' }))
