...
const TERMS: Record<string, string> = {
  'Kommunikasjonsmodellen': 'Modell som beskriver kommunikasjonsprosessen: sender → budskap → kanal → mottaker → tilbakemelding.',
  'Støy': 'Alt som forstyrrer overføringen av et budskap, enten det er fysisk, psykologisk eller kulturelt.',
  'Verbal kommunikasjon': 'Kommunikasjon gjennom ord, enten muntlig eller skriftlig.',
  'Ikke-verbal kommunikasjon': 'Kommunikasjon gjennom kroppsspråk, mimikk, tonefall og holdning.',
  'Aktiv lytting': 'Å gi full oppmerksomhet til den som snakker, og vise det gjennom blikk, nikk og oppfølgingsspørsmål.',
  'Digital dialog': 'Kommunikasjon via digitale kanaler som e-post, chat og sosiale medier.',
  'Jeg-budskap': 'Kommunikasjonsteknikk der man uttrykker egne følelser og opplevelser i stedet for å anklage den andre.',
  'Mangfoldskompetanse': 'Evnen til å kommunisere respektfullt og effektivt med mennesker fra ulike kulturer og bakgrunner.',
}
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva er støy i kommunikasjonsmodellen?',
    options: [
      'Høy bakgrunnsmusikk som forstyrrer en samtale fysisk',
      'Alt som forstyrrer eller forvrenger overføringen av budskapet',
      'Dårlig mobildekning under en telefonsamtale',
      'Grammatikkfeil og skrivefeil i tekst',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er ikke-verbal kommunikasjon?',
    options: [
      'Skriftlig kommunikasjon via e-post og brev',
      'Kommunikasjon gjennom kroppsspråk, tonefall og gester',
      'Digital kommunikasjon på sosiale medier',
      'Tausheten og fraværet av kommunikasjon',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Å lytte til musikk og podcaster med full konsentrasjon',
      'Å gi full oppmerksomhet og aktivt bekrefte at du forstår budskapet',
      'Å ta notater av alt som sies under et møte',
      'Å avbryte med oppfølgingsspørsmål underveis',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er et jeg-budskap i kommunikasjon?',
    options: [
      'En teknikk der man forteller om egne prestasjoner og ferdigheter',
      'En kommunikasjonsteknikk der man uttrykker egne følelser fremfor å anklage den andre',
      'En direktiv kommunikasjonsform fra leder til ansatt',
      'En markedsføringsteknikk der man personifiserer budskapet',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er mangfoldskompetanse?',
    options: [
      'Å beherske mange ulike dataverktøy og systemer',
      'Evnen til å kommunisere respektfullt og effektivt med mennesker fra ulike kulturer',
      'Å ha kompetanse innen mange ulike fagfelt',
      'Kunnskap om internasjonale handelsavtaler og regelverk',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Kommunikasjon<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>og kundebehandling</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 21 – Kommunikasjon, lytting og mangfoldskompetanse i service.</p>
...
function Slide1({ show: _show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 21 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Hva er kommunikasjon?</h2>
          <p style={bodyText}>Det er å dele informasjon slik at vi forstår hverandre likt. Det handler like mye om å lytte som om å snakke.</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1573497620053-ea5300f94f21?auto=format&fit=crop&w=800&q=80" alt="Kommunikasjon" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 900, margin: '0 auto' }}>
        <SlideLabel>Modul 21 – Slide 2</SlideLabel>
        <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 32, lineHeight: 1.1 }}><Term word="Kommunikasjonsmodellen" onShow={show} /></h2>
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', flexWrap: 'wrap', alignItems: 'center' }}>
          {['Sender', '→', 'Budskap', '→', 'Kanal', '→', 'Mottaker', '→', 'Tilbakemelding'].map((item, i) => (
            item === '→' ? (
              <span key={i} style={{ color: '#38bdf8', fontSize: 24, fontWeight: 800 }}>{item}</span>
            ) : (
              <div key={i} style={{ padding: '1rem 1.5rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, fontSize: 15, fontWeight: 600, color: '#38bdf8' }}>
                {item}
              </div>
            )
          ))}
        </div>
        <p style={{ ...bodyText, marginTop: 32, fontSize: 16 }}>Alle ledd i modellen er like viktige for å sikre god forståelse.</p>
      </div>
    </Glass>
  )
}
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=800&q=80" alt="Støy i kommunikasjon" />
        <div>
          <SlideLabel>Modul 21 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Støy" onShow={show} /> – Hva ødelegger?</h2>
          <p style={bodyText}>Alt som forstyrrer samtalen.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Bråk i lokalet (fysisk støy)', 'Dårlig humør (psykologisk støy)', 'Ulike språk (kulturell støy)'].map(item => (
              <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#cbd5e1', fontSize: 16 }}>
                <span style={{ color: '#f87171', fontWeight: 800 }}>✕</span> {item}
              </li>
            ))}
          </ul>
        </div>
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 900, margin: '0 auto' }}>
        <SlideLabel>Modul 21 – Slide 4</SlideLabel>
        <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 32, lineHeight: 1.1 }}><Term word="Verbal kommunikasjon" onShow={show} /> vs. <Term word="Ikke-verbal kommunikasjon" onShow={show} /></h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
          <div style={{ padding: '2rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.25)', borderRadius: 20 }}>
            <div style={{ fontWeight: 800, color: '#38bdf8', fontSize: 18, marginBottom: 16 }}>Verbal (Ord)</div>
            <p style={{ color: '#cbd5e1', fontSize: 16, lineHeight: 1.6, margin: 0 }}>Hva du sier – ordene og setningene dine</p>
          </div>
          <div style={{ padding: '2rem', background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.25)', borderRadius: 20 }}>
            <div style={{ fontWeight: 800, color: '#4ade80', fontSize: 18, marginBottom: 16 }}>Ikke-verbal (Kroppsspråk)</div>
            <p style={{ color: '#cbd5e1', fontSize: 16, lineHeight: 1.6, margin: 0 }}>Blikk, tonefall, holdning – betyr ofte mer enn ordene</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide5({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 21 – Slide 5</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Aktiv lytting" onShow={show} /></h2>
          <p style={bodyText}>Å vise med hele deg at du følger med: Nikke, ha blikkontakt og stille oppfølgingsspørsmål. Da føler kunden seg sett.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Nikk og bekreft', 'Oppretthold blikkontakt', 'Still oppfølgingsspørsmål'].map(item => (
              <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#cbd5e1', fontSize: 16 }}>
        ...
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Aktiv lytting" />
      </div>
    </Glass>
  )
}
...
function Slide6({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=800&q=80" alt="Digital dialog" />
        <div>
          <SlideLabel>Modul 21 – Slide 6</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Digital dialog" onShow={show} /></h2>
          <p style={bodyText}>På e-post og chat ser vi ikke kroppsspråket. Da er det ekstra viktig å være høflig og tydelig for å unngå misforståelser.</p>
        </div>
      </div>
    </Glass>
  )
...
function Slide7({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 21 – Slide 7</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Mangfoldskompetanse" onShow={show} /></h2>
          <p style={bodyText}>Folk fra ulike land har ulike regler for høflighet. En god servicemedarbeider tilpasser seg gjesten.</p>
          <div style={{ marginTop: 20, padding: '1.25rem 1.5rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 14 }}>
            <p style={{ color: '#38bdf8', fontWeight: 600, fontSize: 15, margin: 0 }}>Tilpass deg gjestens kulturelle bakgrunn</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1508193638397-1c4234db14d8?auto=format&fit=crop&w=800&q=80" alt="Mangfold og kultur" />
      </div>
    </Glass>
  )
}
...
function Slide8({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1573497620053-ea5300f94f21?auto=format&fit=crop&w=800&q=80" alt="Jeg-budskap" />
        <div>
          <SlideLabel>Modul 21 – Slide 8</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Jeg-budskap" onShow={show} /></h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 8 }}>
            <div style={{ padding: '1.25rem 1.5rem', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 14 }}>
              <p style={{ color: '#f87171', fontWeight: 600, fontSize: 15, margin: 0, fontStyle: 'italic' }}>❌ "Du tar feil" – føles som et angrep</p>
            </div>
            <div style={{ padding: '1.25rem 1.5rem', background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.25)', borderRadius: 14 }}>
              <p style={{ color: '#4ade80', fontWeight: 600, fontSize: 15, margin: 0, fontStyle: 'italic' }}>✓ "Jeg opplever situasjonen slik..." – åpner for dialog</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Analyse</SlideLabel>
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Hvorfor er smilet ditt det viktigste verktøyet du har i en butikk?</h2>
        <p style={{ ...bodyText, fontSize: 20 }}>Tenk på hva ikke-verbal kommunikasjon gjør med kundenes opplevelse og vilje til å komme tilbake.</p>
      </div>
    </Glass>
  )
}

export default function KommunikasjonPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Kommunikasjon',
    }))
    setLivePin(pin)
