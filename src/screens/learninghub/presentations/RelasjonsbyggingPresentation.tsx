...
const TERMS: Record<string, string> = {
  'Relasjonsbygging': 'Systematisk arbeid med å etablere og vedlikeholde tillitsfulle forretningsforbindelser.',
  'Formelt nettverk': 'Organiserte nettverk med tydelig struktur og formål, f.eks. bransjeforeninger og næringslivsklubber.',
  'Uformelt nettverk': 'Personlige forbindelser skapt gjennom uformell kontakt, f.eks. kaffeprater og messebesøk.',
  'B2B': 'Business to Business – handel mellom to bedrifter, i motsetning til B2C (bedrift til forbruker).',
  'LinkedIn': 'Profesjonelt sosialt nettverk for å bygge karriere, vise kompetanse og finne jobbmuligheter.',
  'Tillit': 'Grunnlaget for alle gode forretningsrelasjoner – troen på at den andre parten er pålitelig og ærlig.',
}
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva kjennetegner god relasjonsbygging i næringslivet?',
    options: [
      'Rask og effektiv service uten unødvendig personlig kontakt',
      'Å skape langvarige og tillitsfulle forbindelser med kunder og partnere',
      'Aggressiv markedsføring og hyppig kontakt for mersalg',
      'Å konkurrere aktivt mot andre aktører i markedet',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er det viktigste grunnlaget for å bygge tillit?',
    options: [
      'Alltid å tilby lavest mulig pris i markedet',
      'Konsistens, ærlighet og å holde det man lover',
      'Rask levering og bredt produktsortiment',
      'Å tilby flest mulig produkter og tjenester',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er profesjonell nettverksbygging?',
    options: [
      'Installasjon og vedlikehold av datanettverksutstyr',
      'Å etablere og pleie profesjonelle forbindelser som kan gi gjensidig verdi',
      'Å bruke sosiale medier kun for privat underholdning',
      'Å ansette mange medarbeidere med ulik bakgrunn',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er et formelt nettverk?',
    options: [
      'Et privat nettverk av venner og familie',
      'Et organisert nettverk med tydelig struktur og formål, som bransjeforeninger',
      'Et digitalt nettverk på sosiale medier',
      'En intern kommunikasjonsplattform i bedriften',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva skiller B2B fra B2C?',
    options: [
      'B2B er netthandel, B2C er handel i fysisk butikk',
      'B2B er handel mellom bedrifter, B2C er handel mellom bedrift og forbruker',
      'B2B gjelder tjenester, B2C gjelder produkter',
      'B2B er internasjonalt, B2C er lokalt',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Relasjons-<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>bygging</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 19 – Relasjonsbygging og nettverk i salg og service.</p>
...
function Slide1({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 19 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Relasjoner er kapital</h2>
          <p style={bodyText}>I salg og service handler alt om hvem du kjenner og hvem som stoler på deg. Gode <Term word="Relasjonsbygging" onShow={show} />srelasjoner gjør at kunder kommer tilbake igjen og igjen.</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=800&q=80" alt="Relasjoner er kapital" />
      </div>
    </Glass>
  )
}
...
function Slide2({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=800&q=80" alt="Formelle nettverk" />
        <div>
          <SlideLabel>Modul 19 – Slide 2</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Formelt nettverk" onShow={show} /></h2>
          <p style={bodyText}>Organisasjoner som NHO Reiseliv der bedrifter møtes for å lære av hverandre og samarbeide om å lokke turister til Norge.</p>
          <div style={{ marginTop: 20, padding: '1.25rem 1.5rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 14 }}>
            <p style={{ color: '#38bdf8', fontWeight: 600, fontSize: 15, margin: 0 }}>Eksempel: NHO Reiseliv, Innovasjon Norge</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 19 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Uformelt nettverk" onShow={show} /></h2>
          <p style={bodyText}>Det er den "gode praten" over en kaffe eller på en messe. Her bygges <Term word="Tillit" onShow={show} /> som ikke står i noen kontrakt.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Kaffe med kollegaer', 'Messebesøk og bransjetreff', 'Sosiale medier og LinkedIn'].map(item => (
        ...
        <SlideImg src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=800&q=80" alt="Uformelt nettverk" />
      </div>
    </Glass>
  )
}
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1521791136064-7986c2920216?auto=format&fit=crop&w=800&q=80" alt="B2B salg" />
        <div>
          <SlideLabel>Modul 19 – Slide 4</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="B2B" onShow={show} />-salg</h2>
          <p style={bodyText}>B2B betyr Business to Business. Når en bedrift skal kjøpe 50 biler av en annen bedrift, er det personlige nettverket og <Term word="Tillit" onShow={show} /> helt avgjørende for om det blir et salg.</p>
        </div>
      </div>
    </Glass>
  )
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 19 – Slide 5 – Case</SlideLabel>
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Case: Innovasjon Norge</h2>
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 24, padding: '2.5rem' }}>
          <p style={{ ...bodyText, fontSize: 18, margin: 0 }}>De arrangerer store workshops og messer der små norske bedrifter får møte store utenlandske reisebyråer for å lage avtaler.</p>
        </div>
        <p style={{ color: '#64748b', fontSize: 15, marginTop: 20, fontStyle: 'italic' }}>Resultat: Norsk reiseliv vokser gjennom målrettet nettverksbygging.</p>
      </div>
    </Glass>
  )
}
...
function Slide6({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 19 – Slide 6</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="LinkedIn" onShow={show} /> – Ditt digitale ansikt</h2>
          <p style={bodyText}>Det viktigste verktøyet for å bygge et profesjonelt nettverk. Her viser du frem hva du kan, og du kan bli kontaktet av mulige arbeidsgivere.</p>
          <ul style={{ ...listStyle, marginTop: 20 }}>
            {['Bygg en sterk profil', 'Del faglig innhold', 'Knytt kontakter i bransjen'].map(item => (
              <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#cbd5e1', fontSize: 16 }}>
                <span style={{ color: '#38bdf8', fontWeight: 800 }}>✓</span> {item}
              </li>
            ))}
        ...
        <SlideImg src="https://images.unsplash.com/photo-1508193638397-1c4234db14d8?auto=format&fit=crop&w=800&q=80" alt="LinkedIn og digitalt nettverk" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, marginBottom: 32, lineHeight: 1.2 }}>Er det sant at "hvem du kjenner" er viktigere enn karakterene dine når du skal søke jobb?</h2>
        <p style={{ ...bodyText, fontSize: 20 }}>Diskuter fordelene og ulempene ved at nettverk spiller en stor rolle i arbeidsmarkedet.</p>
      </div>
    </Glass>
  )
}

export default function RelasjonsbyggingPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Relasjonsbygging',
    }))
    setLivePin(pin)
