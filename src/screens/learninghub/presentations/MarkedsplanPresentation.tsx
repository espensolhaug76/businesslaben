...
const TERMS: Record<string, string> = {
  'Markedsplan': 'Et strategisk dokument som beskriver bedriftens markedsaktiviteter for en periode.',
  'Arbeidsbetingelser': 'De forholdene bedriften opererer under. Kan være interne (kontrollerbare) eller eksterne (ukontrollerbare).',
  'Interne arbeidsbetingelser': 'Faktorer bedriften selv kontrollerer: ressurser, kompetanse, produkter og økonomi.',
  'Eksterne arbeidsbetingelser': 'Faktorer utenfor bedriftens kontroll: lover, konkurrenter, trender og økonomi.',
  'SOFT-analyse': 'Analyse av Styrker, svakheter (Oppfatninger), Forutsetninger (muligheter) og Trusler. Også kalt SWOT.',
  'SMART-mål': 'Målsettingsmetode: Spesifikk, Målbar, Attraktiv, Realistisk og Tidsbestemt.',
  'Offensiv strategi': 'Å bruke en styrke til å utnytte en mulighet i markedet.',
  'Defensiv strategi': 'Å bruke en styrke til å beskytte seg mot en trussel.',
}
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva er interne arbeidsbetingelser?',
    options: [
      'Faktorer utenfor bedriftens kontroll som lovverk og konkurrenter',
      'Faktorer bedriften selv kan påvirke, som ressurser og kompetanse',
      'Markedstrender og endringer i forbrukeratferd',
      'Makroøkonomiske faktorer som renter og inflasjon',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva betyr S i SMART-mål?',
    options: [
      'Strategisk – at målet støtter bedriftens overordnede strategi',
      'Spesifikt – at målet er klart og tydelig definert',
      'Salgsorientert – at målet er knyttet til salgsvolum',
      'Synlig – at målet er kommunisert til alle ansatte',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva analyserer man i en SOFT-analyse?',
    options: [
      'Salg, Omsetning, Fortjeneste og Tid',
      'Styrker, svakheter, muligheter og trusler',
      'Segmentering, Offerings, Feedback og Timing',
      'Strategi, Organisasjon, Finans og Teknologi',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er en offensiv strategi i SOFT-analysen?',
    options: [
      'Å angripe konkurrenter direkte med aggressiv prissetting',
      'Å bruke en intern styrke til å utnytte en ekstern mulighet',
      'Å forsvare bedriften mot trusler fra omgivelsene',
      'Å redusere kostnader i en nedgangsperiode',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva skiller en markedsplan fra en bedriftsplan?',
    options: [
      'Markedsplanen er kortere og mindre detaljert enn bedriftsplanen',
      'Markedsplanen fokuserer spesifikt på markedsaktiviteter, mens bedriftsplanen dekker hele virksomheten',
      'De er i praksis det samme dokumentet med ulikt navn',
      'Bedriftsplanen er kun for interne brukere, markedsplanen er offentlig',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Markeds<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>plan</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 13 – Situasjonsanalyse og strategisk planlegging for bedriften.</p>
      {_isLive ? (
...
function Slide1({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 13 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Veikartet til suksess</h2>
          <p style={bodyText}>En <Term word="Markedsplan" onShow={show} /> er et dokument som forteller hva bedriften skal gjøre det neste året for å nå målene sine (f.eks. selge mer eller bli mer kjent).</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=800&q=80" alt="Markedsplan og strategi" />
      </div>
    </Glass>
  )
}
...
function Slide2({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=800&q=80" alt="Arbeidsbetingelser" />
        <div>
          <SlideLabel>Modul 13 – Slide 2</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Arbeidsbetingelser" onShow={show} /> – Rammen din</h2>
          <p style={bodyText}>Det er forholdene bedriften må jobbe under. Noe kan du styre, andre ting må du bare tåle.</p>
        </div>
      </div>
    </Glass>
  )
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 13 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Interne arbeidsbetingelser" onShow={show} /> – "Inne i huset"</h2>
          <p style={bodyText}>Det er ting bedriften kontrollerer selv: ansatte, penger i banken, produktene og ledelsen. Dette blir bedriftens styrker eller svakheter.</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=800&q=80" alt="Interne faktorer" />
      </div>
    </Glass>
  )
}
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?auto=format&fit=crop&w=800&q=80" alt="Eksterne faktorer og trender" />
        <div>
          <SlideLabel>Modul 13 – Slide 4</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Eksterne arbeidsbetingelser" onShow={show} /> – "Utafor vinduet"</h2>
          <p style={bodyText}>Det er ting du ikke kan styre: lover, været, trender på TikTok eller hva konkurrenten gjør. Dette skaper muligheter eller trusler.</p>
        </div>
      </div>
    </Glass>
  )
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 900, margin: '0 auto' }}>
        <SlideLabel>Modul 13 – Slide 5</SlideLabel>
        <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="SOFT-analyse" onShow={show} /> (SWOT)</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginTop: 24 }}>
          {[
            { label: 'S – Styrker', desc: 'Hva er vi gode på?', color: '#38bdf8' },
            { label: 'O – svakheter', desc: 'Hva må vi forbedre?', color: '#f87171' },
            { label: 'F – Muligheter', desc: 'Hva kan vi gripe?', color: '#4ade80' },
            { label: 'T – Trusler', desc: 'Hva må vi passe oss for?', color: '#fb923c' },
          ].map(item => (
            <div key={item.label} style={{ padding: '2rem', background: 'rgba(255,255,255,0.05)', borderRadius: 20, borderTop: `3px solid ${item.color}`, textAlign: 'center' }}>
              <div style={{ fontWeight: 800, color: item.color, fontSize: 20, marginBottom: 8 }}>{item.label}</div>
              <div style={{ color: '#94a3b8', fontSize: 16 }}>{item.desc}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <Glass>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <SlideLabel>Modul 13 – Slide 6</SlideLabel>
        <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Strategisk bruk – Hvordan vinne?</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
          <div style={{ padding: '2rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.25)', borderRadius: 20 }}>
            <div style={{ fontWeight: 800, color: '#38bdf8', fontSize: 18, marginBottom: 12 }}><Term word="Offensiv strategi" onShow={show} /> (S+M)</div>
            <p style={{ color: '#cbd5e1', fontSize: 15, lineHeight: 1.6, margin: 0 }}>Bruk en styrke til å gripe en mulighet. Eks: Vi er best på sosiale medier + kundene bruker mer tid på TikTok = Kjør på med reklame der!</p>
          </div>
          <div style={{ padding: '2rem', background: 'rgba(251,146,60,0.08)', border: '1px solid rgba(251,146,60,0.25)', borderRadius: 20 }}>
            <div style={{ fontWeight: 800, color: '#fb923c', fontSize: 18, marginBottom: 12 }}><Term word="Defensiv strategi" onShow={show} /> (S+T)</div>
            <p style={{ color: '#cbd5e1', fontSize: 15, lineHeight: 1.6, margin: 0 }}>Bruk en styrke til å beskytte deg mot en trussel. Eks: Vi har de mest lojale kundene + en ny konkurrent åpner = Minn kundene på hvorfor de elsker oss.</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide7({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 13 – Slide 7</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Målsetting – <Term word="SMART-mål" onShow={show} /></h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
            {[
              { letter: 'S', word: 'Spesifikk' },
              { letter: 'M', word: 'Målbar' },
              { letter: 'A', word: 'Attraktiv' },
              { letter: 'R', word: 'Realistisk' },
              { letter: 'T', word: 'Tidsbestemt' },
            ].map(item => (
              <div key={item.letter} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '0.5rem 1rem', background: 'rgba(56,189,248,0.08)', borderRadius: 10 }}>
                <span style={{ fontWeight: 800, color: '#38bdf8', fontSize: 22, minWidth: 28 }}>{item.letter}</span>
                <span style={{ color: '#cbd5e1', fontSize: 16 }}>{item.word}</span>
              </div>
            ))}
          </div>
          <p style={{ color: '#64748b', fontSize: 14, fontStyle: 'italic' }}>Eks: "Øke salget med 10 % innen jul"</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=800&q=80" alt="SMART målsetting" />
      </div>
    </Glass>
  )
}
...
function Slide8() {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1557838923-2985c318be48?auto=format&fit=crop&w=800&q=80" alt="Markedsstrategi" />
        <div>
          <SlideLabel>Modul 13 – Slide 8</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Markedsstrategi</h2>
          <p style={bodyText}>Valg av målgruppe og hvordan vi skal bruke de 5 P-ene for å nå målene.</p>
        </div>
      </div>
    </Glass>
  )
...
function Slide9() {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 13 – Slide 9</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Aktivitetsplan og budsjett</h2>
          <p style={bodyText}>Hva skal skje når, og hva koster det?</p>
          <div style={{ marginTop: 24, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              { col: 'Aktivitet', val: 'Hva skal gjøres?' },
              { col: 'Tidsfrist', val: 'Når skal det skje?' },
              { col: 'Ansvarlig', val: 'Hvem gjør det?' },
              { col: 'Kostnad', val: 'Hva koster det?' },
            ].map(item => (
              <div key={item.col} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '0.75rem 1rem', background: 'rgba(56,189,248,0.08)', borderRadius: 10 }}>
                <span style={{ fontWeight: 700, color: '#38bdf8', fontSize: 14, minWidth: 90 }}>{item.col}</span>
                <span style={{ color: '#cbd5e1', fontSize: 15 }}>{item.val}</span>
              </div>
            ))}
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=800&q=80" alt="Aktivitetsplan" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 13 – Slide 10 – Case</SlideLabel>
        <h2 style={{ fontSize: 52, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Case: PåPåler AS</h2>
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 24, padding: '2.5rem', marginBottom: 24 }}>
          <p style={{ ...bodyText, fontSize: 20 }}>En bedrift som driver med glamping. De fant ut at styrken deres var unike hytter, men trusselen var dårlig vær.</p>
        </div>
        <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: '1.5rem' }}>
          <p style={{ color: '#38bdf8', fontWeight: 700, fontSize: 18, margin: 0 }}>Valgt strategi: "Koselig inne uansett vær" for å lokke kunder.</p>
        </div>
      </div>
    </Glass>
  )
...
export default function MarkedsplanPresentation() {
