...
const TERMS: Record<string, string> = {
  'Kjøpsprosessen': 'De fem stegene en kunde går gjennom: behovserkjennelse, informasjonssøk, vurdering, kjøp og evaluering.',
  'Maslows behovspyramide': 'Modell som rangerer menneskelige behov fra grunnleggende (fysiologiske) til selvrealisering.',
  'Segmentering': 'Å dele opp markedet i grupper med felles karakteristikker for å nå dem mer effektivt.',
  'Demografisk segmentering': 'Inndeling etter målbare fakta som alder, kjønn, inntekt og utdanning.',
  'Psykografisk segmentering': 'Inndeling etter livsstil, verdier, interesser og personlighetstyper.',
  'Atferdssegmentering': 'Inndeling basert på kjøpsatferd, lojalitet og bruksmønster.',
  'Persona': 'En detaljert, fiktiv profil av en typisk kunde i målgruppa. Brukes for å forstå kundens behov bedre.',
  'Målgruppe': 'Den spesifikke gruppen potensielle kunder en bedrift retter sin markedsføring mot.',
}
const QUIZ_SLIDES: QuizQuestion[] = [
  {
    question: 'Hva er det første steget i kjøpsprosessen?',
    options: [
      'Informasjonssøk om tilgjengelige alternativer',
      'Behovserkjennelse – å oppdage at man har et udekket behov',
      'Evaluering av ulike produktalternativer',
      'Selve kjøpsbeslutningen og gjennomføringen',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'Segmentering basert på demografiske faktorer som alder og kjønn',
      'Segmentering basert på livsstil, verdier og personlighet',
      'Segmentering basert på geografisk lokasjon',
      'Segmentering basert på konkret kjøpsatferd og lojalitet',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'En ansatt i markedsavdelingen med særlig kundekompetanse',
      'En detaljert, fiktiv beskrivelse av en typisk målgruppekunde',
      'En reklamestrategi rettet mot beslutningstakere',
      'En type markedsundersøkelse basert på observasjon',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    ...
    options: [
      'En markedsføringsstrategi for å nå ulike kundegrupper',
      'En modell som rangerer menneskelige behov fra grunnleggende til selvrealisering',
      'Et verktøy for å analysere konkurrenters prissetting',
      'En metode for å kartlegge markedsandeler',
    ],
    correct: 1,
    timeSeconds: 45,
  },
  {
    question: 'Hva er atferdssegmentering?',
    options: [
      'Segmentering basert på alder, kjønn og inntekt',
      'Segmentering basert på geografisk beliggenhet',
      'Segmentering basert på kjøpsatferd, lojalitet og bruksmønster',
      'Segmentering basert på livsstil og personlighetstype',
    ],
    correct: 2,
    timeSeconds: 45,
  },
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: 'clamp(3rem, 10vw, 8rem)', fontWeight: 800, marginBottom: 32, letterSpacing: '-0.03em', lineHeight: 1.05 }}>
        Forbruker<br /><span style={{ color: '#38bdf8', fontStyle: 'italic' }}>atferd</span>
      </h1>
      <p style={{ fontSize: 22, color: '#94a3b8', marginBottom: 48, maxWidth: 600, margin: '0 auto 48px', fontWeight: 300, lineHeight: 1.6 }}>Modul 11 – Hvorfor handler vi, og hvordan deler vi markedet inn i målgrupper?</p>
      {_isLive ? (
...
function Slide1() {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 11 – Slide 1</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}>Hvorfor handler vi?</h2>
          <p style={bodyText}>Vi styres av følelser, venner, trender og behov. Noen handler logisk (trenger nye sokker), andre emosjonelt (vil ha de samme skoene som en kjendis).</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=800&q=80" alt="Shopping og forbrukere" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 900, margin: '0 auto' }}>
        <SlideLabel>Modul 11 – Slide 2</SlideLabel>
        <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 32, lineHeight: 1.1 }}><Term word="Kjøpsprosessen" onShow={show} /></h2>
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          {[
            { step: '1', label: 'Behovserkjennelse', desc: 'Sulten' },
            { step: '2', label: 'Informasjonssøk', desc: 'Hvor er det mat?' },
            { step: '3', label: 'Vurdering', desc: 'Pizza eller burger?' },
            { step: '4', label: 'Kjøp', desc: 'Bestiller!' },
            { step: '5', label: 'Evaluering', desc: 'Ble du fornøyd?' },
          ].map((item, i) => (
            <div key={i} style={{ padding: '1.25rem 1.25rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16, minWidth: 140, textAlign: 'center' }}>
              <div style={{ fontSize: 28, fontWeight: 800, color: '#38bdf8', marginBottom: 6 }}>{item.step}</div>
              <div style={{ fontWeight: 700, color: '#fff', fontSize: 14, marginBottom: 4 }}>{item.label}</div>
...
function Slide3({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 11 – Slide 3</SlideLabel>
          <h2 style={{ fontSize: 36, fontWeight: 800, marginBottom: 16, lineHeight: 1.1 }}><Term word="Maslows behovspyramide" onShow={show} /></h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { level: '5', title: 'Selvrealisering', desc: 'Hobbyer og drømmer (Hurtigruten/Utdanning)', color: '#38bdf8' },
              { level: '4', title: 'Status/Anerkjennelse', desc: 'Dyre merker som viser hvem du er (Gant/Rolex)', color: '#818cf8' },
              { level: '3', title: 'Sosiale', desc: 'Vennskap og tilhørighet (Snapchat/TikTok)', color: '#a78bfa' },
              { level: '2', title: 'Trygghet', desc: 'Forsikring eller en trygg bil (Volvo)', color: '#c084fc' },
              { level: '1', title: 'Fysiologiske', desc: 'Mat og drikke (Coca-Cola)', color: '#e879f9' },
            ].map(item => (
              <div key={item.level} style={{ padding: '0.75rem 1rem', background: 'rgba(255,255,255,0.05)', borderRadius: 12, borderLeft: `4px solid ${item.color}`, display: 'flex', gap: 12, alignItems: 'center' }}>
                <span style={{ fontWeight: 800, color: item.color, fontSize: 18, minWidth: 20 }}>{item.level}</span>
                <div>
                  <div style={{ fontWeight: 700, color: '#fff', fontSize: 15 }}>{item.title}</div>
        ...
        <SlideImg src="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=800&q=80" alt="Forbrukeratferd" />
      </div>
    </Glass>
  )
}
...
function Slide4({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1557838923-2985c318be48?auto=format&fit=crop&w=800&q=80" alt="Segmentering" />
        <div>
          <SlideLabel>Modul 11 – Slide 4</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Segmentering" onShow={show} /> – Kaken deles opp</h2>
          <p style={bodyText}>Du kan ikke selge til alle samtidig. Å segmentere betyr å dele markedet inn i grupper som ligner på hverandre (f.eks. "gamere", "småbarnsforeldre" eller "pensjonister").</p>
        </div>
      </div>
    </Glass>
  )
...
function Slide5({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 11 – Slide 5</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Demografisk segmentering" onShow={show} /></h2>
          <p style={bodyText}>Du deler inn etter fakta som alder, kjønn og inntekt.</p>
          <div style={{ marginTop: 24, padding: '1.5rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16 }}>
            <p style={{ color: '#94a3b8', fontSize: 15, margin: 0 }}>Eksempel: Bleier selges til de som har babyer.</p>
          </div>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=800&q=80" alt="Demografisk segmentering" />
      </div>
    </Glass>
  )
}
...
function Slide6({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <SlideImg src="https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?auto=format&fit=crop&w=800&q=80" alt="Psykografisk segmentering" />
        <div>
          <SlideLabel>Modul 11 – Slide 6</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Psykografisk segmentering" onShow={show} /></h2>
          <p style={bodyText}>Du deler inn etter livsstil og verdier.</p>
          <div style={{ marginTop: 24, padding: '1.5rem', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 16 }}>
            <p style={{ color: '#94a3b8', fontSize: 15, margin: 0 }}>Eksempel: En sportsbutikk treffer de som elsker å være ute i skogen.</p>
          </div>
        </div>
      </div>
    </Glass>
...
function Slide7({ show }: { show: (t: string, d: string) => void }) {
  return (
    <Glass>
      <div style={twoCol}>
        <div>
          <SlideLabel>Modul 11 – Slide 7</SlideLabel>
          <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Atferdssegmentering" onShow={show} /></h2>
          <p style={bodyText}>Du ser på hvordan folk handler. Er de lojale til ett merke? Handler de bare når det er salg?</p>
        </div>
        <SlideImg src="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=800&q=80" alt="Atferd og kjøpsmønster" />
      </div>
    </Glass>
  )
}
  ...
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 11 – Slide 8</SlideLabel>
        <h2 style={{ fontSize: 42, fontWeight: 800, marginBottom: 24, lineHeight: 1.1 }}><Term word="Persona" onShow={show} /> – Møt "Tur-Trine"</h2>
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 24, padding: '2.5rem', textAlign: 'left' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            {[
              { label: 'Navn', value: 'Tur-Trine' },
              { label: 'Alder', value: '24 år' },
              { label: 'Yrke', value: 'Student' },
              { label: 'Interesse', value: 'Elsker fjellet' },
              { label: 'Økonomi', value: 'Lite penger' },
              { label: 'Behov', value: 'Utstyr som varer' },
            ].map(item => (
              <div key={item.label} style={{ padding: '1rem', background: 'rgba(255,255,255,0.05)', borderRadius: 12 }}>
                <div style={{ fontSize: 11, color: '#38bdf8', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>{item.label}</div>
                <div style={{ color: '#fff', fontWeight: 600 }}>{item.value}</div>
              </div>
            ))}
          </div>
        </div>
        <p style={{ ...bodyText, marginTop: 24, fontSize: 16 }}>Når du vet hvem hun er, er det lettere å skrive reklame som hun liker!</p>
      </div>
    </Glass>
  )
}
...
function Slide9() {
  return (
    <Glass>
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto' }}>
        <SlideLabel>Modul 11 – Slide 9 – Diskusjon</SlideLabel>
        ...
        <div style={{ background: 'rgba(56,189,248,0.08)', border: '2px dashed rgba(56,189,248,0.3)', borderRadius: 24, padding: '3rem' }}>
          <p style={{ fontSize: 26, color: '#e2e8f0', lineHeight: 1.6, fontWeight: 300, fontStyle: 'italic' }}>
            "Hvordan påvirker TikTok-trender dine valg når du kjøper klær eller mat?"
          </p>
        </div>
      </div>
    </Glass>
...
export default function ForbrukeratferdPresentation() {
  ...
  function startLiveSession() {
    ...
    localStorage.setItem(`adventure-live-session-${pin}`, JSON.stringify({
      ...
      status: 'active',
      presentationName: 'Forbrukeratferd',
    }))
    setLivePin(pin)
